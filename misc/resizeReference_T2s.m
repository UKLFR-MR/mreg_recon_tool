...
% T2s: Bruno Riemenschneider, fixes and changes
...
%% Reco with combination of (resized) cmaps

reference.combreco = squeeze(sum(reference.cmaps.*repmat(conj(reference.smaps),[1 1 1 1 size(reference.cmaps,5)]),4));
...
try
    ...
    if strcmp(reference.mode,'3d') || strcmp(reference.mode,'multi_slice') % 3D
        ...
        reference.wmap = mri_field_map_reg(squeeze(pmaps),te,'l2b',-1,'winit',reference.wmap,'mask',reference.mask);
    end
catch
    ...
    if strcmp(reference.mode,'3d') || strcmp(reference.mode,'multi_slice') % 3D
        reference.wmap = mri_field_map_reg3D(pmaps,te,'l2b',-1,'mask',reference.mask);
    else                                                                   % 2D
        reference.wmap = mri_field_map_reg(squeeze(pmaps),te,'l2b',-1,'mask',reference.mask);
    end
end
...
x = [ones(1,size(reference.cmaps,5)); -te-mean(-te)]';
reference.combreco(reference.combreco==0) = max(reference.combreco(:))*1e-6;
tmp = log(permute(abs(reference.combreco),[4 1 2 3]));
%n = 6;
%tmp = pinv(x(1:n,:))*tmp(1:n,:);
tmp = pinv(x(:,1:end))*tmp(1:end,:);
tmp = reshape(tmp,[2 newRes]);
tmp(tmp<0) = 0;
reference.R2s = squeeze(tmp(2,:,:,:)).*reference.mask;
reference.R2s = smooth3(reference.R2s,'gaussian',3,0.8);
clear tmp

%reference.R2s = log(abs(reference.SENSEreco(:,:,:,1))./abs(reference.SENSEreco(:,:,:,end)))/(te(end)-te(1));
%reference.R2s(reference.R2s<0) = 0;
%reference.R2s = smooth3(reference.R2s,'gaussian',3,0.8);
%reference.R2s = reference.R2s.*reference.mask;
