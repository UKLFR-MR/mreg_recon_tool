function [z, z_iter] = regularizedReconstruction(A,volume,varargin)
...
%         z0 = A'*b;
%        gamma = max(col(abs(A*z0))) / max(abs(b(:)));
        gamma = max(col(abs(A'*(A*z0)))) / max(abs(volume(:)));
        z0 = z0 / gamma; % adjust the amplitude of the initial guess according to the data 'b'
    end
    
%    reference_norm = l2norm(A'*b);

    reference_norm = l2norm(volume);
    ...
    for k=1:length(I_fhandle)
        ...
        if I_all(n+1)==I_all(n)+2;
            R{counter} = @(z) varargin{I_fhandle(k)+1}^2*((1./pc).*z);
            counter = counter + 1;
        else
    ...
    if nargout==2
%        [z, z_iter, n_it] = conjugateGradient_pc(S,q,tol,maxit,z0,verbose_flag,newfigure,pc);
        [z, z_iter] = conjugateGradient(S,q,tol,maxit,z0,verbose_flag,newfigure);
    else
%        [z, ~, n_it] = conjugateGradient_pc(S,q,tol,maxit,z0,verbose_flag,newfigure,pc);
