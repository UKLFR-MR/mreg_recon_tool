 function [wmap, wconv] = mri_field_map_reg3D(yi, etime, varargin)
...
%
% in
%	yik	[nx,ny,nz,nsets]	complex images at different echo times
...
if nargin >= 1 && streq(yi, 'test')
	wmap = mri_field_map_reg_test(yi, varargin{:});
...
if isempty(arg.mask)
	arg.mask = logical(ones(nx,ny,nz));
else
	jf_equal(size(arg.mask), [nx ny nz])
end

% scale data to make regularization effect invariant to scaling factors
[yi y2] = mri_field_map_reg_scale(yi, etime, arg);

% Reshape y for use in SPS & in calculating cost
ycol = reshape(yi, nx*ny*nz, nsets); ycol = ycol(arg.mask, :);
ycol2 = reshape(y2, nx*ny*nz, 2); ycol2 = ycol2(arg.mask, :);
...
if isempty(arg.winit)
    ...
    try
        ...
        warning('You are using an old image reconstruction toolbox. Please download the latest version. Continuing...');
        arg.winit = unwrapping_sps_manysets(wconv(arg.mask), ycol2, ...
...
try
    wmap = unwrapping_sps_manysets(arg.winit(arg.mask), ycol, etime, Rq, 'niter', arg.niter);
catch
    warning('You are using an old image reconstruction toolbox. Please download the latest version. Continuing...');
...
%
function [yi y2] = mri_field_map_reg_scale(yi, etime, arg);
...
wjfactor = zeros(nx,ny,nz);
wjfactor2 = zeros(nx,ny,nz);
for j=1:nsets
	for k=1:nsets
...
totalsum = sum(abs(yi).^2,4) .* abs(yi(:,:,:,1)).^2;
...
xlabel(sprintf('RMS = %.1f Hz', sqrt(mean(error(mask).^2))))
