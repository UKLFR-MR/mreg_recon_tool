...
%% Connect trajectories by succesive removal of points until smooth
%connection is possible 
function [G exitflag]=find_optimal_grad3D_remove_points(SYS,T1,T2)
    
    alpha_step = pi/10;
    N1=0; N2=0; N3 = 5;% N_in=0; N_out = 10;
    exitflag=zeros(3,1);
    ...
    while ~( exitflag(1) == 1 && exitflag(2) == 1 && exitflag(3) == 1)
        ...
        if N1 < 5 && N2 < 5
            if N1 < size(T1.K,1) && N2 < size(T2.K,1)
                N1=N1+1; N2=N2+1;
            elseif N1 < size(T1.K,1)
                N1=N1+1;
            elseif N2 < size(T2.K,1)
                N2=N2+1;
            else
                N3=N3+1;
%                error('Can''t connect by removing points. Both trajectories are too short and too far apart.');
            end
        elseif T1.in_out==1 && T2.in_out==1
            if N1 < size(T1.K,1)
                N1=N1+1;
            else
                N3=N3+1;
...
%            end
        ...
        alpha = 0;
        while alpha < 2*pi-alpha_step
            T2 = trajectStruct_rotate(T2,alpha_step,[0 0 1]);
