% VDS_PNS  Variable density spiral generation obeying PNS limits
%
% THIS CODE IS A MODIFICATION OF BRIAN HARGREAVES FREELY AVALIABLE vds.m
% mrsrl.stanford.edu/~brian/vdspiral/
%
% [k,g,s,time,r,theta] = vds_pns(smax,gmax,T,N,Fcoeff,rmax,pthresh,chronaxie,SRmin)
%
%
...
%		rmax = 1/(2*resolution);
%   chronaxie= Chronaxie time constant   [s]
%   SRmin=rheobase/alpha= min slewrate causing stimulation [T/m/s]
%
%	OUTPUTS:
%	--------
...
function [k,g,s,time,r,theta] = vds_pns(smax,gmax,T,N,Fcoeff,rmax,...
    pthresh,chronaxie,SRmin)
...
r = zeros(1,ml);
time = zeros(1,ml);
kk = zeros(1,ml/oversamp);
SR = zeros(1,ml/oversamp);
pt = zeros(1,ml/oversamp);

ss = smax;
while r0 < rmax
	% [q2,r2] = findq2r2(smax,gmax,r0,r1,To,T,N,Fcoeff,rmax);
    [q2,r2] = findq2r2(ss,gmax,r0,r1,To,T,N,Fcoeff,rmax);
    ...
    % calculate PNS
    if (rem(count,oversamp)==0) && ~isempty(pthresh),
        kk(l) = r0*exp(1i*q0)/gamma/100;
        if l>2,
            SR(l) = (kk(l) - 2*kk(l-1) + kk(l-2))/T.^2;
            pt(l) = 100*T*chronaxie/SRmin*...
                sum(SR(1:l)./(chronaxie+((l:-1:1)-0.5)*T).^2,2);
        end
        if l>5
             if (2*abs(pt(l))-abs(pt(l-5)))>pthresh
                ss = ss*0.98;
            end
        end
        if abs(pt(l))>pthresh-0.2,
%             if abs(pt(l))>pthresh-0.36,
%                 ss = ss*0.99;
%             else
                ss = ss*0.9995;
%             end
            if abs(pt(l))>pthresh, ss = ss*0.995; end
        else
            % if abs(pt(l))<pthresh-0.23, ss = ss*1.0002; end
            if abs(pt(l))<pthresh-1, ss = ss*1.02; end
            if abs(pt(l))<pthresh-0.23, ss = ss*1.0002; end
            if ss>smax, ss = smax; end
        end
        l = l+1;
    end
end;
...
% title('k_y vs k_x');
% axis('square');
% axis(1.05*max(abs(k))*[-1 1 -1 1])
% 
% subplot(2,2,2);
% plot(time,real(k),'r--',time,imag(k),'b--',time,abs(k),'k-');
% title('k vs t');
% ylabel('k (cm^{-1})');
% axis([0 time(end) -1.05*max(abs(k)) 1.05*max(abs(k))])
% 
% subplot(2,2,3);
% plot(time,real(g),'r--',time,imag(g),'b--',time,abs(g),'k-');
% title('g vs t');
% ylabel('G (G/cm)');
% axis([0 time(end) -1.05*max(abs(g)) 1.05*max(abs(g))])
% 
% subplot(2,2,4);
% plot(time,real(s),'r--',time,imag(s),'b--',time,abs(s),'k-');
% title('s vs t');
% ylabel('Slew Rate (G/cm/s)');
% axis([0 time(end) -1.05*max(abs(s)) 1.05*max(abs(s))])
...
%         fprintf('Slew violation, slew = %d, smax = %d, sr=%f, r=%f, r1=%f',...
%             round(abs(slew)),round(smax),sr,r,r1);
%     end;
