function out = write_ak_wav(fname,grad,bw,fov,desc,n_kpts,rfs_fastfid,...
    ak_dynwf_mode)
...
if ~exist('rfs_fastfid','var'), rfs_fastfid = []; end
if isempty(rfs_fastfid), rfs_fastfid = -10; end
if ~exist('ak_dynwf_mode','var'), ak_dynwf_mode = []; end
if isempty(ak_dynwf_mode), ak_dynwf_mode = 0; end
...
if N.intl>1,
    tmp = sum(grad.^2,1);
%     [tmp,sos_ind] = max(tmp,[],2);
%     wf4pg(:,1:N.groups) = sos_ind-1;
    for l1=1:N.groups,
        sos_val = 0;
        for l2=1:N.intl,
...
params = [grad_type fov N.intl str2num(sprintf('%4.6f',gmax)) N.gpts ...
    gdt N.kpts kdt 0 (rfs_fastfid+10) ak_dynwf_mode wf4pg];
% hw_limits removed
