...
% BR: modified some code for the DORK correction. The actual implementation only works for trajectories of equal segment length!

% modified for gfactor noise simulation: use as usual, you need to load an 
% MREG scan which
% is used for the noise correlations, and then the frames' raw data are all replaced
% with noise. The noise is uncorrelated, and the sensitivity maps are
% transformed to the uncorrelated combinations.
% -Bruno Riemenschneider 2018
...
%delay in mus for delay correction
...
%            [D.trajectory, status] = loadTrajectory(filename, []);
        ...
        D.recon.recon_details.DORK_k0 = D.trajectory.idx{1}(idx_k0);        % this implementation only works for 1 segment!
...
%%%%%%%%%%%%
    function z0_pb_cb(hobj,eventdata)
        if all([D.check.rawdata_loaded_flag, D.check.reference_loaded_flag, D.check.trajectory_loaded_flag])
            ...
            D.gui_data.z0 = double(mreg_recon_tool_noise_simulation_recon('local',data,recon_details));
...
%        if all([D.check.rawdata_loaded_flag, D.check.reference_loaded_flag, D.check.trajectory_loaded_flag, D.check.adjust_reference_and_trajectory_flag])
        if all([D.check.reference_loaded_flag, D.check.trajectory_loaded_flag, D.check.adjust_reference_and_trajectory_flag])
            if D.gui_data.add_penalty_counter==0
                local_set_status('At least one penalty has to be added first.');
                return;
            end
...
%                    mreg_recon_tool_sge_init(data,recon_details,D.param.frames_per_job);
                    mreg_recon_tool_noise_simulation_slurm_init(data,recon_details,D.param.frames_per_job);
...
%                    if strcmp(err.identifier, 'mreg_recon_tool_sge_init:Dir_exists')
                ...
                D.recon.recon = mreg_recon_tool_noise_simulation_recon('local',data,recon_details);
