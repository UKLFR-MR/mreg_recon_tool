...
% 21.09.2011
% 
% modified for gfactor noise simulation: you need to load a MREG scan which
% is used for the noise correlations, and the frames' raw data are then all replaced
% with noise.
...
for k=1:length(tframes)
    ...
    % create uncorrelated noise (channelwise)
    
    rng('shuffle') % initialize randomization seed
    b = randn(Ni*length(data.trajectory.idx{1}),size(data.smaps,4)) + 1i*randn(Ni*length(data.trajectory.idx{1}),size(data.smaps,4));
    b = b/sqrt(2); %-> std(b) = 1
    b_corr = (inv(rawdata_header.noise.covariance_matrix_inv_sqrt)*b.').';
%    b_uncorr = (header.noise.covariance_matrix_inv_sqrt*b_corr.').';
    b_uncorr = b;
    
    % create forward operator with uncorrelated channels
...
%    if isempty(norm_factor)
        norm_factor = norm(b_uncorr(:))/norm(b_corr(:));
%    end
...
%    recon_tf = regularizedReconstruction(A,b_corr(:),P{:}, ...
