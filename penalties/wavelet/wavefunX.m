function [out1,out2,out3,out4,out5] = wavefunX(wname,in2,in3);
...
[wtype,fname,family,bounds] =  ...
    wavemngrX('fields',wname,'type','file','fn','bounds');
...
switch wtype
    case 1
      [Lo_R,Hi_R] = wfiltersX(wname,'r');
      long  = length(Lo_R);
      nbpts = (long-1)/pas+1;
      phi   = coef*upcoefX('a',1,Lo_R,'dummy',iter);
      psi   = coef*upcoefX('d',1,Lo_R,Hi_R,iter);

      [nbpts,nb,dn] = getNBpts(nbpts,iter,long);
      phi = [0 wkeep1X(phi,nb) zeros(1,1+dn)];
      psi = [0 wkeep1X(psi,nb) zeros(1,1+dn)];

      % sign depends on wavelet
      if strcmp(debut,'co') | strcmp(debut,'sy') | ... % coiflet or symlet
         strcmp(debut,'dm')                            % dmeyerX
          psi = -psi ;
      end
      out1 = phi; out2 = psi;
    ...
    case 2
      [Lo_D1,Hi_D2,Lo_R2,Hi_R1,Lo_D2,Hi_D1,Lo_R1,Hi_R2] = wfiltersX(wname);

      mul = 1;
      isBior = wavemngrX('isbior',wname);
      if isBior
          Nr = wstr2numX(wname(5));
          if rem(Nr,4)~=1 , mul = -1; end
      end

      long  = length(Lo_D1);
      nbpts = (long-1)/pas;
      phi1  = coef*upcoefX('a',1,Lo_R1,'dummy',iter);
      psi1  = mul*coef*upcoefX('d',1,Lo_R1,Hi_R2,iter);

      [nbpts,nb,dn] = getNBpts(nbpts,iter,long);
      phi1  = [0 wkeep1X(phi1,nb) zeros(1,1+dn)];
      psi1  = [0 wkeep1X(psi1,nb) zeros(1,1+dn)];

      long  = length(Lo_R2);
      Hi_R2 = wrevX(Hi_D2);

      phi2  = coef*upcoefX('a',1,Lo_R2,'dummy',iter);
      psi2  = mul*coef*upcoefX('d',1,Lo_R2,Hi_R1,iter);

      [nbpts,nb,dn] = getNBpts(nbpts,iter,long);
      phi2 = [0 wkeep1X(phi2,nb) zeros(1,1+dn)];
      psi2 = [0 wkeep1X(psi2,nb) zeros(1,1+dn)];
      out1 = phi1; out2 = psi1;
      out3 = phi2; out4 = psi2;
