function out1 = dw2dstatX(option,in2,in3,in4,in5,in6)
...
switch option
    case 'create'
        ...
        % Getting variables from dw2dtoolX figure memory block.
        %-----------------------------------------------------
        ...
        switch s_i
            case 'ss', str_ss = 'Synthesized image';
            case 'ds', str_ss = 'De_noised image';
            case 'cs', str_ss = 'Compressed image';
        end
        str_syn_img = str_ss;
        ...
        for k = 1:Lev_Anal
            str_pop_ad{k}  = sprintf('Level %2.0f', k);
        end
       ...
       rad_det_img = uicontrol(comRadProp{:}, ...
        ...
        % Selected box limits.
        %---------------------
        Axe_Ref = findobj(get(win_dw2dtoolX,'Children'),'flat','Type','axes');
        ...
        str_rad_ori_img = num2mstrX(rad_ori_img);
        str_rad_syn_img = num2mstrX(rad_syn_img);
        str_rad_app_img = num2mstrX(rad_app_img);
        str_rad_det_img = num2mstrX(rad_det_img);
        str_pop_ad      = num2mstrX(pop_ad);
        str_pop_dir     = num2mstrX(pop_dir);
		...
		%----------------------------------------------------
        dw2dstatX('draw',win_stats,win_dw2dtoolX,infos_hdls,selbox);
    ...
    case 'select'
        ...
        [nul,ypixl] = wfigutilX('prop_size',win_stats,0,1); %#ok<ASGLU>
    ...
    case 'draw'
        ...
        if ~isempty(old_params) && (isequal(new_params,old_params)) && ...
                strncmpi(get(axe_hist,'Visible'),'on',2) 
            return
        end
        ...
        elseif app_img
            if sel_cfs
                ...
                curr_img = wrcoef2X('a',coefs,sizes,Wav_Name,level);
                flg_code = 0;
        ...
        elseif det_img
            ...
            if sel_cfs
                curr_img = detcoef2X(dir(1),coefs,sizes,level);
                flg_code = 1;
                str_title= sprintf('Coefficients of detail at level ');
            else
                curr_img = wrcoef2X(dir(1),coefs,sizes,Wav_Name,level);
                flg_code = 1;
        ...
        %-----------------------
        set(win_stats,'Colormap',get(win_dw2dtoolX,'Colormap'));
        ...
        X = wimgcodeX('cod',flg_code,curr_img,NB_ColorsInPal,codemat_v);
        ...
        %----------------------
        wmemtoolX('ini',win_stats,n_miscella,nb3_stored);
        wmemtoolX('wmb',win_stats,n_miscella,  ...
    ...
    case 'update_bins'
        ...
        %------------------------------
        if strncmpi(get(axe_hist,'Visible'),'of',2), return, end
        ...
        % Getting memory blocks.
        %-----------------------
        [curr_img,curr_color] = wmemtoolX('rmb',win_stats,n_miscella,...
