function [out1,out2,out3,out4,out5,out6] = ...
                  dw1dvmodX(option,win_dw1dtoolX,in3,in4,in5,in6,in7,in8,in9)
...
%---------------------------
uic_handles = findobj(win_dw1dtoolX,'type','uicontrol');
...
switch option
    ...
    case 'ch_vm'
        ...
        % Handles of tagged objects.
        %---------------------------
        [btnHeight,ySpacing] = mextglobX('get','Def_Btn_Height','Y_Spacing');
        ...
        hdl_col     = utcolmapX('handles',win_dw1dtoolX,'all','true');
        hdl_col     = hdl_col(:)';
        ...
        if in3==2
            ...
            switch new_mode
              case {2,'2','dec',5,'5','tre'}
                  val = dw1dvmodX('get_vm',win_dw1dtoolX,new_mode);
                  ss_type = wmemtoolX('rmb',win_dw1dtoolX,n_param_anal,ind_ssig_type);
                  str_ss  = ssString(ss_type);
                  set(pus_dispopt, ...
                        'Value',val(2),                   ...
                        'String',sprintf('Show %s Sig.', str_ss) ...
                        );

            end
            switch new_mode
                case {1,'1','scr'} , fname = 'dw1dscrmX';
                case {2,'2','dec'} , fname = 'dw1ddecmX';
                case {3,'3','sep'} , fname = 'dw1dsepmX';
                case {4,'4','sup'} , fname = 'dw1dsupmX';
                case {5,'5','tre'} , fname = 'dw1dtremX';
                case {6,'6','cfs'} , fname = 'dw1dcfsmX';
            end
            feval(fname,'del_ss',win_dw1dtoolX);
            % clean_val = -0;    % reset axes history. 
            clean_val = -1;      % preserve axes history.
            feval(fname,'view',win_dw1dtoolX,clean_val)
        ...
        if ~strcmp(win_units,'pixels')
            [dx,deltay] = wfigutilX('prop_size',win_dw1dtoolX,1,deltay);
        end
        ...
        switch old_mode
            ...
            case {3,'3','sep'}
                ...
                feval('dw1dsepmX','clear',win_dw1dtoolX);
                drawnow;
            ...
            case {4,'4','sup'}
                ...
                feval('dw1dsupmX','clear',win_dw1dtoolX,clear_val);

            case {5,'5','tre'}
                set(pus_dispopt,'Visible','off');
                set(pus_dispopt,'Style','pushbutton');
                feval('dw1dtremX','clear',win_dw1dtoolX);
                drawnow;

            case {6,'6','cfs'}
                set([txt_app_scr,txt_det_scr,pop_app_scr,pop_det_scr, ...
                     pus_dispopt],'Visible','off');
                feval('dw1dcfsmX','clear',win_dw1dtoolX,clear_val);
        end
        ...
        if ((new_mode==2) && find(old_mode==[1 3 4 5 6])) || ...
           ((new_mode==5) && find(old_mode==[1 2 3 4 6]))
            ...
            ss_type = wmemtoolX('rmb',win_dw1dtoolX,n_param_anal,ind_ssig_type);
            str_ss  = ssString(ss_type);
            if new_mode==2 , fname = 'dw1ddecmX'; else fname = 'dw1dtremX'; end 
            cba_disp = [fname '(''ssig'',' ...
                            int2str(win_dw1dtoolX) ',' ...
                            num2mstrX(pus_dispopt) ');'];
            set(pus_dispopt,...
                           'Style','checkbox',...
                           'Value',val(2),...
                           'String',sprintf('Show %s Sig.', str_ss), ...
        ...
        elseif ((old_mode==2) && find(new_mode==[1 3 4 5 6])) || ...
            ...
            cba_disp = ['dw1ddispX(''create'',' int2str(win_dw1dtoolX) ');'];
            set(pus_dispopt,...
                           'Style','pushbutton',            ...
        ...
        switch new_mode
            case {1,'1','scr'}
                ...
                feval('dw1dscrmX','view',win_dw1dtoolX,old_mode);
            ...
            case {2,'2','dec'}
                ...
                feval('dw1ddecmX','view',win_dw1dtoolX,old_mode,lev);
            ...
            case {3,'3','sep'}
                ...
                drawnow
                feval('dw1dsepmX','view',win_dw1dtoolX,old_mode);
            ...
            case {4,'4','sup'}
                ...
                set([pus_dispopt,hdl_col],'Visible','on');
                drawnow
                feval('dw1dsupmX','view',win_dw1dtoolX,old_mode);
            ...
            case {5,'5','tre'}
                ...
                drawnow
                lev = get(pop_lev_dec,'Value');
                feval('dw1dtremX','view',win_dw1dtoolX,old_mode,lev);
            ...
            case {6,'6','cfs'}
                ...
                drawnow
                if (clear_val==0) && (in3==1) , return; end
                feval('dw1dcfsmX','view',win_dw1dtoolX,old_mode);
        end
