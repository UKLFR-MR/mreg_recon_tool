function [Rf,Df] = biorwavfX(wname)
...
lw = length(wname);
Nd = wstr2numX(wname(kdot+1:lw));
wname = wname(1:kdot-1);
lw = length(wname);
ab = abs(wname);
ii = lw+1;
while (ii>1) && (47<ab(ii-1)) && (ab(ii-1)<58), ii = ii-1; end
Nr = wstr2numX(wname(ii:lw));

errStruct = struct(...
    'identifier','Wavelet:Lifting:Invalid_WaveNum', ...
    'message','Invalid wavelet number %d.');

if isempty(find(Nr == [1:5 6], 1))
    error(errStruct.identifier,errStruct.message,Nr);
end
switch Nr
  case 1
      if isempty(find(Nd == 1:2:5, 1))
          error(errStruct.identifier,errStruct.message,Nd);
      end
  case 2
  ...
  case 4 , if Nd ~= 4 , error(errStruct.identifier,errStruct.message,Nd); end
  case 5 , if Nd ~= 5 , error(errStruct.identifier,errStruct.message,Nd); end
  case 6 , if Nd ~= 8 , error(errStruct.identifier,errStruct.message,Nd); end
end
