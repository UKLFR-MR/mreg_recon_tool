function [err,idx] = errlsdecX(LSTable,tolerance)
%ERRLSDEC Errors for liftingX scheme decompositions.
%   For a cell array of liftingX schemes LSTAB (or a single
%   Lifting Scheme LS), 
...
for i=1:nbTST
    ...
    for k=1:nbLS
        LS = LSTable{k};
        x_InPlace = lwtX(x,LS,level);
        xREC = ilwtX(x_InPlace,LS,level);
