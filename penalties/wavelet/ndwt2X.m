function varargout = ndwt2X(X,level,varargin)
...
%       load noiswom; 
%       W1 = ndwt2X(X,2,'db1')
%       W2 = ndwt2X(X,3,'db1','mode','per')
%       W3 = ndwt2X(X,3,{'db1','db2'},'mode','sym')
%       WF = W3.filters
%       W4 = ndwt2X(X,3,WF,'mode','sym')
%
...
for k=1:level
    dec = decFUNC(X,LoD,HiD,dwtXEXTM);
    X = dec{1,1,1};
...
%-------------------------------------------------------------------------%
function dec = decFUNC(X,LoD,HiD,dwtXEXTM)

dec = cell(2,2);
permVect = [];
[a_Lo,d_Hi] = wdec1D(X,LoD{1},HiD{1},permVect,dwtXEXTM);
permVect = [2,1,3];
[dec{1,1},dec{1,2}] = wdec1D(a_Lo,LoD{2},HiD{2},permVect,dwtXEXTM);
[dec{2,1},dec{2,2}] = wdec1D(d_Hi,LoD{2},HiD{2},permVect,dwtXEXTM);
%-------------------------------------------------------------------------%
