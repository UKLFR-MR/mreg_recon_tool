function [Ha,Ga,Hs,Gs,PRCond,AACond] = filters2lpX(type,LoR,varargin)
...
%   In that case, HiR = qmfX(fliplr(LoD))(biorthogonal case).
%
...
switch nbIn
    ...
    case argNUM(4)
        [PmaxHS,AddPOW,qmfXFLAG] = deal(varargin{argNUM(2:4)});
...
switch type
    case 'o' , HiR = qmfX(LoR,qmfXFLAG);
    case 'b' , HiR = qmfX(wrevX(varargin{1}),qmfXFLAG);
end
%--------------------------------------
% The last input argument qmfXFLAG has
...
qmfXPOW = 1 - qmfXFLAG;
Gs = (-1)^qmfXPOW * laurpoly(HiR,PmaxGS);
...
% if  d_LEN ~= 0
    ...
    elseif nbEVEN==1
        ...
        error('Wavelet:FunctionInput:Invalid_BiorFilt',...
