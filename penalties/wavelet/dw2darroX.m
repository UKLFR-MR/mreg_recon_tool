...
axe_handles = findobj(get(win_dw2dtoolX,'Children'),'flat','type','axes');
...
switch option
    case 'ini_arrow'
        fontsize = wmachdepX('fontsize','normal',20);
        axe_arrow = axes(...
        ...
        locProp = {commonProp{:},'string','dwtX','Userdata','t0'};
        text(locProp{:});
        ar2 = wavarrow('ini',axe_arrow,2,Col_BoxTitleSel);
        set(ar2,'Userdata','a2');
        ar3 = wavarrow('ini',axe_arrow,2,Col_ArrowFrm);
        set(ar3,'Userdata','a3');
        locProp = {commonProp{:},'string','idwtX','Userdata','t31'};
        text(locProp{:});
    ...
    case 'set_arrow'
        ...
        [bdx,bdy] = wfigutilX('prop_size',win_dw2dtoolX,15,15);
        mul = 1/6;
        xini = pini(1)+pini(3);
        yini = pini(2);
        xsel = psel(1)-bdx;
        ...
        set(t31,'Position',[xsel-dx/2 ysyn+0.025],'String','idwtX');
...
%WAVARROW Draw arrows and texts for dw2dtoolX.
