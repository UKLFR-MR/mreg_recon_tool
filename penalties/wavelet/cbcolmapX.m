function varargout = cbcolmapX(option,fig,varargin)
...
switch option
    case 'nbc'
        ...
        st = get(e_or_s,'style');
        if strcmp(st,'edit')
            nbcol = wstr2numX(get(e_or_s,'String'));
            continu = 1;
            if isempty(nbcol)
                continu = 0;
        ...
        for k = 1:2:nbarg
           ...
           switch argType
             case 'pal'
                     ...
                     if length(argVal)>1
                         ...
                         if length(argVal)==4
                             ...
                             if ~isempty(udMap)
                                 ...
                                 if isempty(ind)
                                     names = [names;{addNam}]; %#ok<AGROW>
                                 end
                             else
                                 names = mextglobX('get','Lst_ColorMap');
                             end
                 ...
                 %------------------------------
                 namepal = deblanklX(namepal);
                 if ~(isempty(namepal) || isequal(namepal,'same'))
                     ind = find(strcmp(namepal,names));
                 else
    ...
    case 'visible'
        handles = utcolmapX('handles',fig,'all');
