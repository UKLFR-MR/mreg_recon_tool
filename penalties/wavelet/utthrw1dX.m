function varargout = utthrw1dX(option,fig,varargin)
...
switch option
    case 'create'
        ...
        [Def_Txt_Height,Def_Btn_Height,Def_Btn_Width,Pop_Min_Width, ...
         sliYProp,Def_FraBkColor,Def_EdiBkColor] = ...
            mextglobX('get',...
        ...
        mulHeight = wtbutilsX('deno1D_PREFS','mulHeight');
        ...
        str_txt_tit = {'Lev','Int','Select','Thresh'};
        ...
        if levmax<8 , MUL = 1; else MUL = 1; end
        ...
        switch  toolOPT
          case {'deno','esti','esti_REG','esti_DEN'}
          case {'comp'}
            ediBkColor = mextglobX('get','Def_Edi_InActBkColor');
            comEdiProp = {comProp{:}, ...
                'Style','Edit',...
                'String','',...
                'Enable','Inactive', ...
        ...
        switch  toolOPT
          case {'deno','esti','esti_REG','esti_DEN'}
			...
			wfighelpX('add_ContextMenu',fig,hdl_DENO_SOFTHARD,'DENO_SOFTHARD');
			wfighelpX('add_ContextMenu',fig,hdl_DENO_NOISSTRUCT,'DENO_NOISSTRUCT');
		
          case {'comp'}
			hdl_COMP_SCORES = [...
				txt_nor,edi_nor,txt_npc,txt_zer,edi_zer,txt_zpc ...
				];
			wfighelpX('add_ContextMenu',fig,hdl_COMP_SCORES,'COMP_SCORES');
        end
        hdl_COMP_DENO_STRA = [fra_utl,txt_top,pop_met,txt_BMS,sli_BMS];				
		hdl_IDTS_GUI = tog_thr;	
		wfighelpX('add_ContextMenu',fig,hdl_COMP_DENO_STRA,'COMP_DENO_STRA');
		wfighelpX('add_ContextMenu',fig,hdl_IDTS_GUI,'IDTS_GUI');		
		%-------------------------------------
        ...
        varargout{1} = utthrw1dX('set',fig,'position',{levmin,levmax});
    ...
    case 'update_methName'
        ...
        utthrw1dX('update_by_Caller',fig)
        ...
        for k = 1:NB_lev
            ...
            if ~isempty(thrParams)
                ...
                if ishandle(linvalHdl)
                   ...
                   cbthrw1dX('plotLV',[fig ; lu ; ld],[xHOR ; yHOR],yVMin);
                end
            end
        end
        utthrw1dX('set',fig,'thrstruct',thrStruct);
    ...
    case 'update_by_UIC'
        ...
        feval(calledFUN,'clear_GRAPHICS',fig);
        if ok , utthrw1dX('show_LVL_perfos',fig); end
    ...
    case 'update_LVL_meth'
        ...
        maxTHR = utthrw1dX('get',fig,'maxTHR');
        ...
        utthrw1dX('update_hdlDEN',fig,thrStruct);
    ...
    case 'return_SetThr'
        ...
        feval(calledFUN,'clear_GRAPHICS',fig);
        utthrw1dX('update_hdlDEN',fig,varargin{:})
        utthrw1dX('show_LVL_perfos',fig);
    ...
    case 'enable_tog_res'
        enaVal = varargin{1};
        try 
            Pus_SigDorC = wfindobjX(fig,'Tag','Pus_SigDorC');
        catch
            Pus_SigDorC = [];
        end
        ...
        for k = 1:2:nbarg
           ...
           switch argType
             case 'position'
               ...
               utthrw1dX('visible',fig,ud.visible);
               if nargout>0
             ...
             case 'thrstruct'
                 ...
                 wmemtoolX('wmb',fig,n_memblocTHR,ind_thr_struct,argVal);

             case 'intdepthr'
                 wmemtoolX('wmb',fig,n_memblocTHR,ind_int_thr,argVal);
    ...
    case 'get'
        ...
        for k = 1:nbarg
           ...
           switch outType
             ...
             case 'thrstruct'
               varargout{k} = wmemtoolX('rmb',fig,n_memblocTHR,ind_thr_struct);

             case 'intdepthr'
               varargout{k} = wmemtoolX('rmb',fig,n_memblocTHR,ind_int_thr);
    ...
    case 'plot_dec'
        ...
        [lu,ld] = cbthrw1dX('plotLH',[h_CMD_LVL(2:4,lev);h_GRA_LVL(1,lev)], ...
                           xHOR,yHOR,k,max_det);
    ...
    case 'residuals'
        [handleORI,handleTHR,handleRES] = ...
            utthrw1dX('get',fig,'handleORI','handleTHR','handleRES');
        wmoreresX('create',fig,tog_res,handleRES,handleORI,handleTHR,'blocPAR');

    case 'den_M1'
        cfs = varargin{1};
        len = varargin{2};
        thrStruct = utthrw1dX('get',fig,'thrstruct');
        switch toolOPT
          case {'deno','esti','esti_REG','esti_DEN'}
            v = get(rad_sof,'Value');
        ...
        for k = 1:NB_lev
            ...
            if ~isempty(thr_par)
                ...
                for j = 1:NB_int
                    ...
                    j_end = x(j+1);
                    j_ind = (j_beg:j_end);
                    cfs(k,j_ind) = wthreshX(cfs(k,j_ind),sorh,thr(j));
                end
            end
        end
        varargout{1} = cfs;
        ...
        for k = 1:NB_lev
            ...
            if ~isempty(thr_par)
                ...
                for j = 1:NB_int
                    ...
                    j_end = x(j+1);
                    j_ind = (j_beg:j_end);
                    cfs(j_ind) = wthreshX(cfs(j_ind),sorh,thr(j));
                end
    ...
    switch tool
       case {'sw1d','wdre'}
         if ischar(parDemo{1})
             ...
             utthrw1dX('update_methName',fig)

         elseif isnumeric(parDemo{1})
             utthrsetX('demo',tog_thr,parDemo{1})
         end
    end
...
switch option
  case 'names'
     out = strvcat(1,thrMethods{:,1}); %#ok<VCAT>
  ...
  case 'shortnames'
     out = strvcat(1,thrMethods{:,2}); %#ok<VCAT>
     varargout{1} = out(2:end,:);
     if nbin==2
         num = varargin{2};
         varargout{1} = deblank(varargout{1}(num,:));
...
if isequal(toolOPT,'comp') && (nblevs>9)
    [bdy,d_lev] = wtbutilsX('comp1D_PREFS');
else
    [bdy,d_lev] = wtbutilsX('deno1D_PREFS','yParams');
end
%=============================================================================%
