function out1 = dw1dhistX(option,in2,in3,in4,in5,in6,in7,in8)
...
if ~isequal(option,'create') , win_dw1dhistX = in2; end
switch option
    case 'create'
        % Get Globals.
        %-------------
        ...
        [win_dw1dhistX,pos_win,win_units,str_numwin,...
        ...
        pos_show_hist = [px, y_low, 3*push_width/2, hshow];
        ...
        %-----------------------------------------
        if ~isequal(get(0,'CurrentFigure'),win_dw1dhistX)
            figure(win_dw1dhistX);
        end
        utanaparX('create_copy',win_dw1dhistX, ...
                 {'xloc',xlocINI,'bottom',ybottomINI},...
                 {'n_s',{Sig_Name,Sig_Size},'wav',Wav_Name,'lev',Lev_Anal} ...
                 );

        commonProp = {'Parent',win_dw1dhistX,'Unit',win_units};
        ...
        str_set4_hdls     = num2mstrX(set4_hdls);
        str_group_hdls    = num2mstrX([set1_hdls;set2_hdls;set3_hdls;set4_hdls]);
        ...
        %----------------------
        wmemtoolX('ini',win_dw1dhistX,n_chk_lev_lst,nbLOC_1_stored);
        wmemtoolX('wmb',win_dw1dhistX,n_chk_lev_lst,...
        ...
        str_wintitle = [str_wintitle '.  Components :   ' ,...
                        sprintf('%.0f',selbox(1)) ' --> ' ...
                        sprintf('%.0f',selbox(2))];
        wfigtitlX('string',win_dw1dhistX,str_wintitle,'off');

        % Setting units to normalized.
        %-----------------------------
        wfigmngrX('normalize',win_dw1dhistX);
		...
		dw1dhistX('select',win_dw1dhistX,chk_orig_sig,...
			     set1_hdls,set2_hdls,set3_hdls,set4_hdls,group_hdls);		
		dw1dhistX('draw',win_dw1dhistX,win_dw1dtoolX,selbox);
    ...
    case 'select'
        ...
        %---------------------------
        uic_handles  = findobj(win_dw1dhistX,'type','uicontrol');
        ...
        axe_handles  = findobj(get(win_dw1dhistX,'Children'),'flat','type','axes');
    ...
    case 'draw'
        ...
        chk_app_sig   = findobj(chk_handles,'Tag',tag_app_sig);
        chk_det_sig   = findobj(chk_handles,'Tag',tag_det_sig);
        rad_sel_cfs   = findobj(win_dw1dhistX,'Style','radiobutton',...
                                      'Tag',tag_sel_cfs);
        edi_bins_data = findobj(win_dw1dhistX,'Style','edit',...
                                      'Tag',tag_bins_data);
        ...
        [Chk_App_Lst,Chk_Det_Lst] = wmemtoolX('rmb',win_dw1dhistX,n_chk_lev_lst,...
        ...
        %----------------------
        wfigtitlX('vis',win_dw1dhistX,'on');
        ...
        %----------------------------------
        Wav_Name = wmemtoolX('rmb',win_dw1dtoolX,n_param_anal,ind_wav_name);
        ...
        if ~strcmp(win_units,'pixels')
            [nul,btn_height] = wfigutilX('prop_size',win_dw1dhistX,1,btn_height); %#ok<ASGLU>
        end
        pos_win      = get(win_dw1dtoolX,'Position');
        bdx          = 0.1*pos_win(3);
        bdy          = 0.05*pos_win(4);
        ecy          = 0.03*pos_win(4);
        pos_graph    = wmemtoolX('rmb',win_dw1dtoolX,n_miscella,...
                                     ind_graph_area);
        % h_graph      = pos_graph(4);
        w_graph      = pos_graph(3);
        ...
        ss_type = wmemtoolX('rmb',win_dw1dtoolX,n_param_anal,...
                                        ind_ssig_type);
        if     strcmp(ss_type,'ss') , str_ss = 'synthesized signal';
        elseif strcmp(ss_type,'ds'), str_ss = 'de_noised signal';
        elseif strcmp(ss_type,'cs'), str_ss = 'compressed signal';
        end
        ...
        if synt_sig
            ...
            h = txtinaxeX('create',ss_type,axe_left(ind_left),'left',...
                                    'on','bold',fontsize);
            set(h,'Units',win_units);
            set(axe_left(ind_left),'Userdata',curr_sig);
            ind_left = ind_left-1;
        ...
        if app_lst_len~=0 && ~sel_cfs
            rec_apps = dw1dfileX('app',win_dw1dtoolX,1:Lev_Anal);
        end
        for k = app_lst_len:-1:1
            level = app_lst(k);
            ind_coef = 1;
            if sel_cfs
                curr_sig   = appcoefX(coefs,longs,Wav_Name,level);
        ...
        if det_lst_len~=0 && ~sel_cfs
            rec_dets = dw1dfileX('det',win_dw1dtoolX,1:Lev_Anal);
        end
        for k = det_lst_len:-1:1
            level = det_lst(k);
            ind_coef = 1;
            if sel_cfs
                curr_sig   = detcoefX(coefs,longs,level);
                selbox_cfs = selbox_orig;
                min_box    = ceil(min(selbox_cfs)/2^level);
                max_box    = ceil(max(selbox_cfs)/2^level);
                selbox_cfs = min_box:max_box;
        ...
        % Setting units to normalized.
        %-----------------------------
        set(findobj(win_dw1dhistX,'Units','pixels'),'Units','normalized');

        % End waiting.
        %-------------
        set(wfindobjX('figure'),'Pointer','arrow');
        % wwaitingX('off',win_dw1dhistX);
    ...
    case 'update_bins'
        ...
        % Handles of tagged objects.
        %---------------------------
        axes_hdls = findobj(get(win_dw1dhistX,'Children'),'flat','type','axes');
        if isempty(axes_hdls) , return; end
        uic = findobj(get(win_dw1dhistX,'Children'),'flat','type','uicontrol');
        ...
        for i=1:nb_axes
            ...
            if ~isempty(curr_sig)
                ...
                txtinaxeX('create',curr_txt,curr_axe,side,'on','bold',fontsize);
    ...
    case 'demo'
        ...
        %****************************************%
        chk_handles   = findobj(win_dw1dhistX,'Style','checkbox');
        chk_orig_sig  = findobj(chk_handles,'Tag',tag_orig_sig);
        chk_synt_sig  = findobj(chk_handles,'Tag',tag_synt_sig);
        chk_app_sig   = findobj(chk_handles,'Tag',tag_app_sig);
        chk_det_sig   = findobj(chk_handles,'Tag',tag_det_sig);
        pus_show_hist = findobj(win_dw1dhistX,...
                                   'Style','pushbutton',...
                                   'Tag',tag_show_hist...
                                   );
