function wpcf_wcfX(x,lev,wav,colmode,nb_colors,flg_line)
...
nbtn  = length(nodes);
[depths,posis] = ind2depoX(order,nodes);
cfs   = read(wpt,'data');
...
if find(colmode==[1 3 5 7])
    cfs = wcodematX(cfs,nb_colors,'row',abs_val);
end
...
for k = 1:nbtn
    ...
    z = cfs(deb(k):fin(k));
    z = z(ones(1,2^d),:);
    z = wkeep1X(z(:)',NBcoltot);
    if find(colmode==[2 4 6 8])
        z = wcodematX(z,nb_colors,'row',abs_val);
    end
    r1 = ypos(k);
