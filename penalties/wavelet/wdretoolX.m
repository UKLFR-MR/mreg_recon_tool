function varargout = wdretoolX(option,varargin)
...
switch option
  ...
  otherwise
    win_tool = varargin{1};
    handles  = wfigmngrX('getValue',win_tool,'WDRE_handles');
    ...
    colors   = wfigmngrX('getValue',win_tool,'WDRE_colors');
    toolATTR = wfigmngrX('getValue',win_tool,'WDRE_toolATTR');
    switch option
      case {'load','demo'}
...
switch option
    case {'createDEN','createREG'}
        ...
        [win_tool,pos_win,win_units,str_numwin,...
            pos_frame0,Pos_Graphic_Area] = ...
               wfigmngrX('create',win_title,winAttrb,figExtMode,mfilename,1,1,0);
        if nargout>0 , varargout{1} = win_tool; end

        % Menu construction for current figure.
        %--------------------------------------
        m_files = wfigmngrX('getmenus',win_tool);
        switch option
          case 'createREG'
			...
			% Add Help Item.
			%----------------
			wfighelpX('addHelpItem',win_tool,'Regression Estimation','REG_EST');			
			...
			wfighelpX('addHelpItem',win_tool,'Loading and Saving','REG_LOADSAVE');		

			m_load = wfigmngrX('getmenus',win_tool,'load');  
            ...
            uimenu(m_files,...
                'Label','Import from Workspace','Position',4, ...
                'Separator','On',         ...
                'Callback',               ...
                [mfilename '(''load'',' str_numwin ',''wrksreg'',1);'] ...
                );
            m_exp_sig = uimenu(m_files, ...
                'Label','Export Estimated Function to Workspace',   ...
                'Position',5,'Enable','Off','Separator','Off',...
                'Callback',[mfilename '(''exp_wrks'',' str_numwin ',''REG'');']  ...
                );
          ...
          case 'createDEN'
			...
			wfighelpX('addHelpItem',win_tool,'Available Methods','COMP_DENO_METHODS');
			wfighelpX('addHelpItem',win_tool,'Variance Adaptive Thresholding','VARTHR');
			wfighelpX('addHelpItem',win_tool,'Loading and Saving','EDEN_LOADSAVE');		
			wfighelpX('addHelpItem',win_tool,'Density Estimation','DENS_EST');		
            men_fix = [];
            men_sto = [];
            men_den = uimenu(m_files,...
                'Label','&Load data for Density Estimate',...
                'Position',1, ...
                'Callback', ...            ...                
                [mfilename '(''load'',' str_numwin ',''denest'');'] ...
                );
            men_sav = uimenu(m_files,...
                'Label','&Save Density', ...
                'Position',2,           ...
            ...
            uimenu(m_files,...
                ...
                'Separator','On',         ...
                'Callback',               ...
                [mfilename '(''load'',' str_numwin ',''denest'',1);'] ...
                );
            m_exp_sig = uimenu(m_files, ...
                'Label','Export Density to Workspace',   ...
                'Position',5,'Enable','Off','Separator','Off',...
                'Callback',[mfilename '(''exp_wrks'',' str_numwin ',''DEN'');']  ...
                );
        end
        ...
        utanaparX('handles',win_tool,'lev');
        ...
        str_txt = ['a' wnsubstrX(abs(NB_max_lev))];
        txt_app = txtinaxeX('create',str_txt,axe_app,'r',txtLRProp{:});
        ...
        for j = 1:NB_max_lev
            ...
            txt_cfs(k) = txtinaxeX('create',str_txt,axe_cfs(k),'l',txtLRProp{:});
            set(txt_cfs(k),'Userdata',k,'tag','');
        end
        utthrw1dX('set',win_tool,'axes',axe_cfs);
        ...
        for j = 1:NB_max_lev
            ...
            str_txt    = ['d' wnsubstrX(k)];
            txt_det(k) = txtinaxeX('create',str_txt,axe_det(k),'r',txtLRProp{:});
        end
        ...
        % Set default wavelet.
        %---------------------
        cbanaparX('set',win_tool,'wav',default_wave);
		...
		wfighelpX('add_ContextMenu',win_tool,hdl_BINS,helpName);		
		%-------------------------------------
		
        % Memory blocks update.
        %----------------------
        wmemtoolX('ini',win_tool,n_membloc2,nb1_stored);
        wmemtoolX('ini',win_tool,n_membloc1,nb2_stored);
        wmemtoolX('ini',win_tool,n_membloc3,nb4_stored);
        ...
        wfigmngrX('storeValue',win_tool,'WDRE_handles',handles);
        ...
        colors = wtbutilsX('colors','wdre');
        wfigmngrX('storeValue',win_tool,'WDRE_colors',colors);

        toolATTR = struct('toolMode','','toolState','',...
            'level',[],'wname','','NBClasses',[]);
        wfigmngrX('storeValue',win_tool,'WDRE_toolATTR',toolATTR);
    ...
    case {'load','demo'}
        ...
        switch option
          case 'load'
            ...
            if isequal(tool_OPT,'denest') && length(varargin)>2
                [sigInfos,xdata,ok] = wtbximportX('1d');
                loadFLAG = false;
                
            elseif isequal(tool_OPT,'wrksreg')
                [ok,input_VAL,sig_Name] = wtbximportX('reg');
                % [sigInfos,xdata,ok] = wtbximportX('1d');
                loadFLAG = false;
                
            else
                [filename,pathname,ok] = ...
                    utguidivX('test_load',win_tool,'*.mat',dialName);
            end
            if ~ok, return; end
        ...
        if loadFLAG
            ...
            [fileStruct,err] = wfileinfX(fullName);
            if ~err
                try
        ...
        %--------------------------- 
        wmemtoolX('wmb',win_tool,n_membloc2,ind_status,0);
        ...
        wdretoolX('clean',win_tool,option);
        ...
        %----------------------
        wdretoolX('position',win_tool,lev_Anal);
        wdretoolX('enable',win_tool,'ini','on');
        wdretoolX('set_axes',win_tool);
        ...
        switch tool_OPT
          case 'denest'
            ...
            wtitleX('Data','Parent',axe_L_1)
            color  = colors.denColor;
            his    = wgethistX(xdata,minBIN);
            wplothisX(axe_L_2,his,color);
            strTitle = sprintf('%d bins - Histogram',minBIN);
            wtitleX(strTitle,'Parent',axe_L_2)
            wdretoolX('set_Bins',win_tool);
            
          case {'fixreg','storeg'}
            lin_ori = line(linProp{:},'Xdata',xdata,'Ydata',ydata);
            utthrw1dX('set',win_tool,'handleORI',lin_ori);
            ylim = getylim(ydata);
            set(axe_L_1,'Ylim',ylim)          
            wtitleX('Data (X,Y)','Parent',axe_L_1)
            if tool_OPT(1)=='s' , wdretoolX('set_Bins',win_tool); end
        end

        % Plotting Processed data.
        %------------------------
        wdretoolX('plot_Processed_Data',win_tool);

        % if DEMO, analyze and estimate.
        %-------------------------------
        if isequal(option,'demo')
            wdretoolX('decompose',win_tool);
            if ~isempty(parDemo)
                 utthrw1dX('demo',win_tool,'wdre',parDemo);
            end
            wdretoolX('estimate',win_tool);
            wdretoolX('show_lin_den',win_tool,'On')
        end
    ...
    case 'save'
        ...
        [thrStruct,hdl_den] = utthrw1dX('get',win_tool,...
                                 'thrstruct','handleTHR');
    ...
    case 'exp_wrks'
        ...
        wtbxexportX(S,'name',varNAM,'title',type_EXP);
        wwaitingX('off',win_tool);        
    ...
    case 'decompose'
        ...
        wdretoolX('show_lin_den',win_tool,'off')

        % Get analyzis parameters.
        %-------------------------
        [wname,NB_lev] = cbanaparX('get',win_tool,'wav','lev');
        ...
        % Get X bounds values.
        %---------------------
        xbounds = wmemtoolX('rmb',win_tool,n_membloc1,ind_xbounds);        
        xmin = xbounds(1);  xmax = xbounds(2);

        % Compute.
        %---------
        sig_pro = wmemtoolX('rmb',win_tool,n_membloc3,ind_sig);
        [coefs,longs] = wavedecX(sig_pro,NB_lev,wname);
        % [det,app] = getFullDecTec(coefs,longs,wname,NB_lev);
        wmemtoolX('wmb',win_tool,n_membloc3, ...
                       ind_coefs,coefs, ...
                       ind_longs,longs ...
                       );
        ...
        for k = 1:NB_lev
            maxTHR(k) = max(abs(detcoefX(coefs,longs,k)));
        end
        valTHR = wdretoolX('compute_LVL_THR',win_tool);
        ...
        for j = 1:NB_lev
            ...
            utthrw1dX('plot_dec',win_tool,k,{maxTHR(k),valTHR(k),xmin,xmax,k});
        end
        ...
        set(txt_hdl(end),'String',['a' wnsubstrX(abs(NB_lev))]);
        ...
        dynvtoolX('init',win_tool,axe_IND,axe_CMD,[],[1 0],'','','')
        ...
        % Enabling HDLG.
        %---------------
        wdretoolX('enable',win_tool,'dec','on');

        % Setting prog status.
        %----------------------
        wmemtoolX('wmb',win_tool,n_membloc2,ind_status,1);
    ...
    case 'estimate'
        ...
        %-----------------------
        [coefs,longs] = wmemtoolX('rmb',win_tool,n_membloc3,ind_coefs,ind_longs);
        NB_lev    = toolATTR.level;
        ...
        %------------------------------------
        cden = utthrw1dX('den_M2',win_tool,coefs,longs);
        xbounds = wmemtoolX('rmb',win_tool,n_membloc1,ind_xbounds);
        xmin = xbounds(1); xmax = xbounds(2);
        ...
        % Reset dynvtoolX.
        %----------------
        dynvtoolX('get',win_tool,0,'force');
        ...
        % Enabling HDLG.
        %---------------
        utthrw1dX('set',win_tool,'handleTHR',lin_den);
        utthrw1dX('enable',win_tool,'on');
        wdretoolX('enable',win_tool,'den','on');
    ...
    case 'show_lin_den'
        ...
        wtitleX('Data (X,Y)','Parent',axe_L_1)
        wtitleX('Processed Data (X,Y)','Parent',axe_R_1)

    case 'position'
        NB_lev  = varargin{2};
        set(chk_den,'Visible','off');
        pos_old = utthrw1dX('get',win_tool,'position');
        utthrw1dX('set',win_tool,'position',{1,NB_lev})
    ...
    case 'upd_lev'
        ...
        wdretoolX('position',win_tool,lev_New);
        wdretoolX('set_axes',win_tool);
        ...
        if isequal(lev_New,lev_Anal)
            ...
            switch state
                case 'ini' ,
                    flagView = 1;
                    wdretoolX('enable',win_tool,'ini');
                case 'dec' , 
                    wdretoolX('enable',win_tool,'dec');
                case 'den' ,
    ...
    case 'upd_bin'
        ...
        switch typ_upd
          case 'sli'
            nbOld = round(wstr2numX(get(edi_bin,'String')));
          ...
          case 'edi'
            ...
            nbOld = sliVal{2};
            maxNb = sliVal{3};
            nbNew = round(wstr2numX(get(edi_bin,'String')));
    ...
    case 'compute_LVL_THR'
        ...
        switch toolMode
          case 'denest'
            varargout{1}  = wthrmngrX('dw1ddenoXDEN',meth,coefs,longs,alfa); 
          case {'fixreg','storeg'}
            varargout{1}  = wthrmngrX('dw1ddenoXLVL',meth,coefs,longs,alfa);
        end

    case 'update_LVL_meth'
        wdretoolX('clear_GRAPHICS',win_tool);
        valTHR = wdretoolX('compute_LVL_THR',win_tool);
        utthrw1dX('update_LVL_meth',win_tool,valTHR);
 
    case 'clear_GRAPHICS'
        status = wmemtoolX('rmb',win_tool,n_membloc2,ind_status);
        if status<1 , return; end

        wdretoolX('enable',win_tool,'den','off');
        wdretoolX('show_lin_den',win_tool,'off')
        axe_off = [axe_R_2,axe_app,axe_det];
        axe_off = axe_off(ishandle(axe_off));
        lin_Off = findobj(axe_off,'type','line');
    ...
    case 'enable'
        ...
        switch type
          ...
          case 'dec'
            ...
            utthrw1dX('enable',win_tool,'on');
            utthrw1dX('enable',win_tool,enaVal,1:NB_lev);
          ...
          case 'den'
            ...
            wmemtoolX('wmb',win_tool,n_membloc2,ind_status,status);
        end
    ...
    case 'clean'
        ...
        delete(lin_den);
        wmemtoolX('wmb',win_tool,n_membloc2,ind_lin_den,[NaN,NaN]); 
        switch calling_opt
          case {'load','demo'}
            ...
            sig_name = wmemtoolX('rmb',win_tool,n_membloc1,ind_sig_name);
            cbanaparX('set',win_tool,'nam',sig_name);
            utthrw1dX('clean_thr',win_tool);
    ...
    case 'set_Bins'
        xdata  = wmemtoolX('rmb',win_tool,n_membloc1,ind_xdata);
        ...
        switch toolMode
          case 'denest'
            color  = colors.denColor;
            his    = wgethistX(xdata,nbBINS);
            wplothisX(axe_R_1,his,color);
            wtitleX('Binned Data','Parent',axe_R_1)
          ...
          case 'storeg'
            ...
            his    = wgethistX(xdata,nbBINS);
            wplothisX(axe_L_2,his,color);
            wtitleX('Histogram of X','Parent',axe_L_2)
        end

    case 'plot_Processed_Data'
        [xdata,ydata] = wmemtoolX('rmb',win_tool,n_membloc1, ...
                                       ind_xdata,ind_ydata);
        [sig_pro,xval] = wedenreg(toolATTR,xdata,ydata);
        wmemtoolX('wmb',win_tool,n_membloc3,ind_sig,sig_pro);
        ...
        switch toolMode
          case {'fixreg','storeg'}
            ...
            ylim = getylim(ydata);
            set(axe_L_1,'Ylim',ylim);
            wmemtoolX('wmb',win_tool,n_membloc2,ind_lin_den,lin_den);
    ...
    case 'set_axes'
        ...
        Pos_Graphic_Area = wmemtoolX('rmb',win_tool,n_membloc2,ind_gra_area);
        ...
        if status==0
            txt_app = txt_hdl(end);
            num_app = NB_lev;
            set(txt_app,'string',['a' wnsubstrX(abs(num_app))]);
        end
       ...
       %  set(hdl_den,'color','g')
        ...
        switch toolMode
          case 'fixreg'
            wtitleX('Data (X,Y)','Parent',axe_L_1);
            wtitleX('Regression Estimate Y = f(X)','Parent',axe_R_2);
          ...
          case 'denest'
            wtitleX('Data','Parent',axe_L_1);
            wtitleX(sprintf('%d bins - Histogram',def_MinBIN),'Parent',axe_L_2);
            wtitleX('Binned Data','Parent',axe_R_1);
            wtitleX('Density Estimate','Parent',axe_R_2);
        end
        wtitleX('Details coefficients','Parent',axe_cfs(i_axe_cfs_min));
        wtitleX('Details','Parent',axe_det(i_axe_det_min));
        wtitleX('Approximation','Parent',axe_app);
...
det  = wrmcoefX('d',coefs,longs,wname,1:level);
app  = wrcoefX('a',coefs,longs,wname,level);
if nargout<3 , return; end
sig  = waverecX(coefs,longs,wname);
%-----------------------------------------------------------------------------%
...
%-----------------------------------
if nargout>2
    [coefs,longs] = wavedecX(ysig,level,wname);
end
%============================================================================%
