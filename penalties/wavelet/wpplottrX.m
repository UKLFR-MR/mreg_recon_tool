function wpplottrX(option,win_wptool,Ts,in4,in5)
...
[txt_color,line_color,ftn_size] = ...
                wtbutilsX('wputils','plottree',get(WP_Axe_Tree,'Xcolor'));
order = treeordX(Ts);
depth = treedpthX(Ts);            
all   = allnodesX(Ts);
...
switch option
    ...
    case 'split_merge'
        ...
        [Tree_Lines,Tree_Texts,Tree_Type_TxtV] =...
            wmemtoolX('rmb',win_wptool,n_wp_utils,...
                           ind_tree_lin,ind_tree_txt,ind_type_txt);
        btndown_fcn = get(Tree_Texts(1),'ButtonDownFcn');
                
        % Create the new tree.
        %---------------------
        node  = depo2indX(order,node);
        [d,b] = ind2depoX(order,node);
