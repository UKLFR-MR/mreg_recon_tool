function fact_and_lsX(IN1,IN2,IN3)
%FACT_AND_LS Factorizations and liftingX schemes for wavelets. 
%   === NOT DOCUMENTED ===
...
for k = 1:nb_WAVE_CUR
    ...
    for j = 1:length(powMAX_VAL)
        ...
        for m = 1:length(difPOW_VAL)
            ...
            [Hs,Gs,Ha,Ga] = wave2lpX(wname,powMAX,difPOW);
            ...
            if nbFACT>0
                ...
                [dual_APMF,prim_APMF] = pmf2apmfX(MatFACT,'twoFact');
                
                % Compute Lifting Steps.
                dual_LS = apmf2lsX(dual_APMF);
                prim_LS = apmf2lsX(prim_APMF);
                
                % Control the factorizations.
                [dual_errTAB,dual_errFlags] = errlsdecX(dual_LS,tolerance);
                [prim_errTAB,prim_errFlags] = errlsdecX(prim_LS,tolerance);
                ...
                All_dual_ERR  = [All_dual_ERR     , dual_errTAB]; %#ok<AGROW>
                All_prim_ERR  = [All_prim_ERR     , prim_errTAB]; %#ok<AGROW,AGROW>
                All_dual_POW  = [All_dual_POW     , powMAX*ones(1,nbFACT)]; %#ok<AGROW>
                All_prim_POW  = [All_prim_POW     , powMAX*ones(1,nbFACT)]; %#ok<AGROW>
                All_dual_DIF  = [All_dual_DIF     , difPOW*ones(1,nbFACT)]; %#ok<AGROW>
                All_prim_DIF  = [All_prim_DIF     , difPOW*ones(1,nbFACT)]; %#ok<AGROW>
    ...
    %------------------------------
    eqTAB = tablseqX(All_LS,tolerance);
    toDEL = [];
    for j = 1:length(eqTAB)
        num = eqTAB{j};
        if ~isempty(num) && (num > j)
            toDEL = [toDEL , num]; %#ok<AGROW>
        end
    end
    All_FACT(toDEL) = [];
    All_LS(toDEL)  = [];
