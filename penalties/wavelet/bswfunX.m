function [phiS,psiS,phiA,psiA,xval] = bswfunX(Lo_D,Hi_D,Lo_R,Hi_R,IN5,IN6)
...
coef = (sqrt(2)^iter);
if flagREVERSE  && isequal(Lo_R,wrevX(Lo_D))
    phiA = coef*upcoefX('a',1,wrevX(Lo_D),NaN,iter);
    psiA = coef*upcoefX('d',1,wrevX(Lo_D),wrevX(Hi_D),iter);    
else
    phiA = coef*upcoefX('a',1,Lo_D,NaN,iter);
    psiA = coef*upcoefX('d',1,Lo_D,Hi_D,iter);
end
phiS = coef*upcoefX('a',1,Lo_R,NaN,iter);
psiS = coef*upcoefX('d',1,Lo_R,Hi_R,iter);
...
try set(a,'Xlim',[x(1) x(end)]); catch end
try set(a,'Ylim',[ymin-dy ymax+dy]); catch end
