function varargout = utcolmapX(option,fig,varargin)
    ...
    case {'Install_V3','Install_V3_CB','create'}
        ...
        switch option
            ...
            case 'create'
                ...
                %----------------
                if isnan(ybottom)
                    pos_close = wfigmngrX('get',fig,'pos_close');
                    if ~isempty(pos_close)
                        ybottom = pos_close(2)+pos_close(4)+2*dy;
                    end
                end
                if isnan(bkColor) , bkColor = mextglobX('get','Def_FraBkColor'); end
		...
		%-------------------------------------
		wfighelpX('add_ContextMenu',fig,ud.handles,'UT_COLMAP');
		%-------------------------------------

    case 'handles'
        fra = findobj(fig,'tag',tag_col_par);
        ud  = get(fra,'Userdata');
