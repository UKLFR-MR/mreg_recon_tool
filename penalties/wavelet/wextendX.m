function x = wextendX(type,mode,x,lf,location)
...
switch type
  case {1,'1','1d'}
      ...
      switch mode
        ...
        otherwise
          errargtX(mfilename,'Invalid Extension Mode for DWT!','msg');
  ...
  case {2,'2','2d'}
      ...
      if ndims(x)>2
          y = cell(1,3);
          for k=1:3
              y{k} = wextendX('2D',mode,x(:,:,k),lf,[locRow locCol]);
          end
          x = cat(3,y{:});
          return
      end
      ...
      switch mode
        ...
        otherwise
            errargtX(mfilename,'Invalid Extension Mode!','msg');
            error('Wavelet:Invalid_ArgVal_Or_ArgType', ...
                'Invalid argument value.');
      end
   ...
   case {'ar','ac','addrow','addcol'}
      ...
      if ndims(x)<3
          x = wextendX('2D',mode,x,lf,location);
      else
          y = cell(1,3);
          for k=1:3
              y{k} = wextendX('2D',mode,x(:,:,k),lf,location);
          end
          x = cat(3,y{:});
      end
end
