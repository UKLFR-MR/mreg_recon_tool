function varargout = cwimtoolX(option,varargin)
...
switch option
    case 'create'
		...
		% Add Help for Tool.
		%------------------
		wfighelpX('addHelpTool',win_tool,'&Complex Continuous Analysis','CWIM_GUI');

		% Add Help Item.
		%----------------
		wfighelpX('addHelpItem',win_tool,'Continuous Transform','CW_TRANSFORM');
		wfighelpX('addHelpItem',win_tool,'Continuous Versus Discrete (1)','CW_CONTDISC1');	
		wfighelpX('addHelpItem',win_tool,'Continuous Versus Discrete (2)','CW_CONTDISC2');		
		wfighelpX('addHelpItem',win_tool,'Loading and Saving','CW_LOADSAVE');
		
		% Set 'WindowButtonUpFcn' function.
		%---------------------------------
		cb_up = ['cw1dmngrX(''WindowButtonUpFcn'',' str_numwin ')'];
        ...
        men_load = uimenu(m_files,...
            'Label','&Load Signal', ...
            'Position',1,    ...
            'Callback',['cw1dmngrX(''load'',' str_numwin ');'] ...
            );
        men_save = uimenu(m_files,...
            'Label','&Save Coefficients',...
            'Position',2,                ...
            'Enable','Off',              ...
            'Callback',['cw1dmngrX(''save'',' str_numwin ');'] ...
            );
        men_demo = uimenu(m_files,'Label','&Example Analysis','Position',3);
        men_import = uimenu(m_files,...
            'Label','Import Signal from Workspace', ...
            'Position',4,'Separator','On', ...
            'Callback',['cw1dmngrX(''load'',' str_numwin ',''load_wrks'',1);']...
            );
        men_export = uimenu(m_files,...
            'Label','Export Coefficients to Workspace', ...
            'Position',5,'Enable','Off','Separator','Off', ...
            'Callback',['cw1dmngrX(''exp_wrks'',' str_numwin ');']...
            );
        % String for JA.
        %---------------
        strWITH = sprintf('with ');
        strSCA  = sprintf(' at scales ');

        demoSET = {...
          'Test Singularities (I)' , ...
              'cuspamax' , 'cgau2'       , 1 , [1,1,64] , 128 ; ...
          'Test Singularities (II)' , ...
              'cuspamax' , 'cgau4'       , 1 , [1,1,64] , 128 ; ...
          'Test Singularities (III)' , ...
        ...
        beg_call_str = ['cw1dmngrX(''demo'',' str_numwin ','''];
        ...
        for k=1:nbDEM
            ...
            scales = ['[' num2str(val(1)) ':' num2str(val(2)) ...
                            ':' num2str(val(3)) ']'];
            libel = sprintf([strWITH '%s ' strSCA ' %s   -->  %s '],...
                wav,scales,nam);
        ...
        colmapPos = utcolmapX('position',win_tool);

        % Setting Initial Colormap.
        %--------------------------
        cbcolmapX('set',win_tool,'pal',{'pink',default_nbcolors})

        % UIC construction.
        %------------------------------------------------------------------
        [dy_POS_CCM , dh_CCM , dy_POS_FRA_AXES , dh_FRA_AXES , ...
         chk_Height , pus_Height] = wtbutilsX('cwim_PREFS');                
        y_low       = toolPos(2);
        ...
        %------------------------------------------------------------------
        y_fra       = pos_rad_SCA(2)+pos_rad_SCA(4)+0.5*dy_POS_FRA_AXES;
        ...
        reste = pos_pus_ana(2)-(pos_fra_axe(2)+pos_fra_axe(4));
        y_low = pos_pus_ana(2)-reste/2;
        pos_pus_lin = [xg,y_low,pop_width,pus_Height];
        y_low       = y_low-pus_Height;
        pos_pus_ref = [xg,y_low,pop_width,pus_Height];
        %------------------------------------------------------------------
        ...
        str_pop_sca = {'Step by Step Mode','Power 2 Mode','Manual Mode'};
        str_pus_ana = 'Analyze';
        ...
        %------------------------------
        cba_edi_sam = ['cw1dmngrX(''setSamPer'',' str_numwin ');'];
        cba_pus_ana = ['cw1dmngrX(''anal'',' str_numwin ');'];
        cba_pus_lin = ['cw1dmngrX(''newCfsLine'',' str_numwin ');'];
        cba_pus_ref = ['cw1dmngrX(''newChainLine'',' str_numwin ');'];
        cba_pop_sca = ['cw1dmngrX(''newScaleMode'',' str_numwin ');'];
        cba_axe_RAD = ['cw1dmngrX(''setPosAxesIMAG'',' str_numwin ');'];
        cba_axe_CHK = ['cw1dmngrX(''setPosAxes'',' str_numwin ');'];
        cba_sof_RAD = ['cw1dmngrX(''sca_OR_frq'',' str_numwin ');'];
        cba_pop_ccm = ['cw1dmngrX(''colorCoefs'',' str_numwin ');'];
        ...
        % Callbacks update.
        %------------------
        utanaparX('set_cba_num',win_tool,[m_files;pus_ana]);
        drawnow;
        ...
        %-------------------------------------     
        [hdl_Re_AXES,hdl_Im_AXES] = cw1dutilX('initPosAxes',win_tool,'all',...
                                         Pos_Graphic_Area);
		...
		wfighelpX('add_ContextMenu',win_tool,hdl_CW_SCALES,'CW_SCALES');
		wfighelpX('add_ContextMenu',win_tool,hdl_CW_COEFLINE,'CW_COEFLINE');
		wfighelpX('add_ContextMenu',win_tool,hdl_CW_MAXLINE,'CW_MAXLINE');
		wfighelpX('add_ContextMenu',win_tool,hdl_CW_SCAL2FRQ,'CW_SCAL2FRQ');
		wfighelpX('add_ContextMenu',win_tool,hdl_CW_COLMODE,'CW_COLMODE');		
		%-------------------------------------
        ...
        wmemtoolX('ini',win_tool,n_coefs_sca,nb2_stored);
        wmemtoolX('wmb',win_tool,n_param_anal,ind_gra_area,Pos_Graphic_Area);
        wmemtoolX('wmb',win_tool,n_coefs_sca,ind_sca_OR_frq,ind_scales);
        ...
        wfigmngrX('storeValue',win_tool,'CW1D_handles',handles);
