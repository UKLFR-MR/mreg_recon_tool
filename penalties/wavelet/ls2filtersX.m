function varargout = ls2filtersX(LS,OPTION)
...
[cfsGs,degGs] = get(Gs,'coefs','maxDEG');
[cfsGa,degGa] = get(Ga,'coefs','maxDEG');
POW_Hi_R = mod(1-degHs,2);
POW_Hi_D = mod(1-degHa + length(cfsHa),2);
Lo_R = cfsHs;
Lo_D = wrevX(cfsHa);
Hi_R = (-1)^POW_Hi_R * cfsGs;
Hi_D = (-1)^POW_Hi_D * wrevX(cfsGa);
% Lo_R = get(Hs,'coefs');
% Hi_R = get(Gs,'coefs');
