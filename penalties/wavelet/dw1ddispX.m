...
switch option
    case 'create'
        ...
        win_units = 'pixels';
        [nul,nul,win_height,win_width] = wfigmngrX('figsizes');
        old_units = get(win_dw1dtoolX,'Units');
        set(win_dw1dtoolX,'Units','pixels');
        pos_wcall = get(win_dw1dtoolX,'Position');
        set(win_dw1dtoolX,'Units',old_units);
        height0 = pos_wcall(4);
        ...
        win_dw1d_more  = wfindobjX('figure','tag',tag_win_disp);
        fig_name = ['More Display Options : Fig.' sprintf('%.0f',win_dw1dtoolX)];
        if ~isempty(win_dw1d_more)
            old_win = get(win_dw1d_more,'Userdata');
            if old_win~=win_dw1dtoolX
                msg = ['Fig. ' sprintf('%.0f',old_win) ' : ' ...
                        'Keep Display Options ?' ];

                % Test for keeping display options.
                %----------------------------------
                new = wwaitansX(old_win,msg,1,'cancel');
                if new==-1 , return; end
                if new==0  , dw1ddispX('cancel',old_win,win_dw1d_more); end

                dw1dutilX('enable',old_win,'end_more_disp');
                figure(win_dw1dtoolX);  
            end
        end
        dw1dutilX('enable',win_dw1dtoolX,'more_disp');
        if ~isempty(win_dw1d_more)
            hdl_in  = findobj(win_dw1d_more,'type','uicontrol');
            delete(hdl_in);
            set(win_dw1d_more,'Name',fig_name,'Userdata',win_dw1dtoolX);
        else
            ...
            set(win_dw1d_more,...
                    ...
                    'BackingStore','off',   ...
                    'Userdata',win_dw1dtoolX,...
                    'Tag',tag_win_disp      ...
                    );
            wfigmngrX('extfig',win_dw1d_more,'ExtFig_More');
            set(win_dw1d_more,'Renderer','painters','RendererMode','auto')
			
			% Add Help for Tool.
			%------------------
			wfighelpX('addHelpTool',...
        ...
        switch ss_type
            case 'ss' , str_ss_app = 'Synth.';
            case 'ds' , str_ss_app = 'Deno.';
            case 'cs' , str_ss_app = 'Comp.';
        end
        ...
        %------------------------------
        str_wincall = sprintf('%.0f',win_dw1dtoolX);
        ...
        switch num_mode
            case {1,6}
                ...
                [flg_axe,flg_sa,flg_app,flg_sd,flg_det,ccfs_m] = ...
                                dw1dvmodX('get_vm',win_dw1dtoolX,num_mode);
                val_flg = [flg_axe(1) flg_sa flg_axe(2) flg_sd];
                vis_str = getonoffX([1 flg_axe([1 1]) 1 flg_axe([2 2])]);
                val_cfs = flg_axe(3);
                if val_cfs==0
                ...
                dw1ddispX('ena_pop',win_dw1dtoolX,flg_axe(1:2));
            ...
            case 3
                ...
                vis_str = getonoffX([1 l_s_flg l_s_flg 1 r_s_flg r_s_flg ]);
                if val_cfs==0
                    vis_txt_ccfs = 'off'; vis_pop_ccfs = 'off';
                else
                    vis_txt_ccfs = 'on';  vis_pop_ccfs = 'on';
        ...
        if find(num_mode==[1 3 4 6])
			...
			wfighelpX('add_ContextMenu',win_dw1d_more,hdl_CSHelp,'CW_COLMODE');
			%-------------------------------------
    ...
    case 'apply'
        ...
        switch num_mode
            case {1,6}
                dw1dvmodX('set_vm',win_dw1dtoolX,num_mode,...
        ...
        switch num_mode
            case 1 , fname = 'dw1dscrmX';
            case 2 , fname = 'dw1ddecmX';
            case 3 , fname = 'dw1dsepmX';
            case 4 , fname = 'dw1dsupmX';
            case 5 , fname = 'dw1dtremX';
            case 6 , fname = 'dw1dcfsmX';
        end
        feval(fname,'view',win_dw1dtoolX,-1);
    ...
    case 'cancel'
        ...
        switch num_mode
            case {1,6}
                [flg_axe,flg_sa,flg_app,flg_sd,flg_det,ccfs_m] = ...
                                dw1dvmodX('get_vm',win_dw1dtoolX,num_mode,0);
                val_flg = [flg_axe(1) flg_sa flg_axe(2) flg_sd];
                vis_str = getonoffX([1 flg_axe([1 1]) 1 flg_axe([2 2])]);
                val_cfs = flg_axe(3);
        ...
        %-------------------------------
        dw1ddispX('apply',win_dw1dtoolX,win_dw1d_more);

    case 'close'
        delete(in3);
        dw1dutilX('enable',win_dw1dtoolX,'end_more_disp');
