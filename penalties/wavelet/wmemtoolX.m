function varargout = wmemtoolX(option,varargin)
...
%   WMEMTOOL('close')
%   WMEMTOOL('empty')
%   WMEMTOOL('put',NAME,VAL) or wmemtoolX('put',NAME,VAL,STRINFO)
    ...
    case 'wmb'
        ...
        if okFig
            ...
            if ~ok_bloc
                ...
                t = wmemtoolX('ini',fig,varargin{2},nbval);
            end
            bloc    = get(t,'Userdata');
...
%------------------------%
    ...
    case 'put'
        ...
        %------------------------------------
        fig = findobj(0,'type','figure','tag',tag_fig_mem);
        if isempty(fig) , fig = wmemtoolX('create'); end
        varargout{1} = wmemtoolX('wmb',fig,varargin{:});
        varargout{2} = fig;
    ...
    case 'get'
        ...
        fig = findobj(0,'type','figure','tag',tag_fig_mem);
        if ~isempty(fig)
            [varargout{1:length(varargin)-1}] = wmemtoolX('rmb',fig,varargin{:});
        else
            varargout{1} = [] ;
        end
    ...
    case 'del'
        ...
        %*************************%
        fig = findobj(0,'type','figure','tag',tag_fig_mem);
        if ~isempty(fig), wmemtoolX('dmb',fig,varargin{:}); end
