function varargout = cmddenoiseX(sig,wname,level,sorh,nb_IntVal,thrParams)
...
%     [sden,cfs] = cmddenoiseX('nblocr1','db5',5,'s',3);
%     subplot(2,1,1); plot(sig,'r'); axis tight
%     hold on; plot(sden,'k');
%     subplot(2,1,2); plot(sden,'k'); axis tight
%
...
%     load nblocr1; sig = nblocr1;
%     load Sav_Den_Sig
%     [sden,cfs] = cmddenoiseX('nblocr1','db5',5,'s',3,thrParams);
%     subplot(2,1,1); plot(sig,'r'); axis tight
%     hold on; plot(sden,'k');
%     subplot(2,1,2); plot(sden,'k'); axis tight
%
% See also wdencmpX, wdenX, wthrmngrX.
...
% Wavelet Analysis.
%------------------
[coefs,longs] = wavedecX(sig,level,wname);
siz = size(coefs);
if isequal(siz(2),1), coefs = coefs';end
if ~iscell(thrParams)
    [thrParams,int_DepThr_Cell] = utthrsetX_cmd(coefs,longs,nb_IntVal);
end
...
sigden = waverecX(coefs,longs,wname);
