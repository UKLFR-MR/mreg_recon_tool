function varargout = utnbcfsX(option,fig,varargin)
...
switch option
  ...
  otherwise
    ...
    Hdls_toolPos = utanaparX('handles',fig,'all');
    ind          = 2;
    ...
    %----------------------
    Hdls_Sel = wfigmngrX('getValue',fig,'Hdls_Sel');
    Hdls_Mov = wfigmngrX('getValue',fig,'Hdls_Mov');
    ...
    switch option
      case {'clean','enable','Init_Movie','Mngr_Movie'}
        toolATTR = wfigmngrX('getValue',fig,'ToolATTR');
...
switch option
  case 'create'
    ...
    %------------------------
    [bdy,d_lev,mulHeight] = wtbutilsX('utnbCFS_PREFS');
    btnHeight = Def_Btn_Height*mulHeight;
    NB_Height = 6;
    w_fra   = mextglobX('get','Fra_Width');
    h_fra   = (levmaxMAX+NB_Height)*Def_Btn_Height+...
               levmaxMAX*d_lev+ btnHeight+(NB_Height-1)*bdy;
    x_fra   = utposfraX(xleft,xright,xloc,w_fra);
    y_fra   = utposfraX(ybottom,ytop,yloc,h_fra);
    pos_fra = [x_fra,y_fra,w_fra,h_fra];

    % String properties.
    %-------------------
    str_txt_top = 'Define Selection method';
    str_txt_tit = {' ','Initial','  ','Kept'};
    str_tog_res = 'Residuals';
    str_pop_met = {'Global','By Level','Manual','Stepwise Movie'};
    if isequal(toolOPT,'cf2d') , str_pop_met(3) = []; end
    str_txt_app = 'App. cfs';
    str_pop_app = {'Select All','Selectable','Unselect'};
    ...
    for j=1:length(index)
        ...
        switch i
            ...
            case -levmax
                ...
                col = wtbutilsX('colors','app','text');
            otherwise
                str_lev = sprintf('D%.0f',i);
                col = wtbutilsX('colors','det','text');
        end
        ...
        strHdl  = num2mstrX([edi_ini,sli_lev,edi_lev]);
        beg_cba = [mfilename '(''update_by_UIC'',' str_numfig ...
            ',' strHdl];
        cba_sli = [beg_cba ',''sli'',' int2str(i) ');'];
        cba_edi = [beg_cba ',''edi'',' int2str(i) ');'];
    ...
    wfigmngrX('storeValue',fig,'Hdls_Sel',Hdls_Sel);
    wfigmngrX('storeValue',fig,'Hdls_Mov',Hdls_Mov);
    ...
    wfighelpX('add_ContextMenu',fig,hdl_CSHelp,helpName);
    %-------------------------------------

    varargout{1} = utnbcfsX('set',fig,'position',{levmin,levmax});
  ...
  case 'clean'
    nameMeth = utnbcfsX('get',fig,'nameMeth');
    switch nameMeth
      case 'Stepwise' , utnbcfsX('Mngr_Movie',fig,pus_mov_can);
      case {'ByLevel','Manual'}   , utnbcfsX('update_methode',fig,'clean');
    end
    dum1 = h_CMD_SIG([2,4],:);
    dum2 = h_CMD_APP([2,4],:);
    dum3 = h_CMD_LVL([2,4],:);
    ...
    for k = 1:2:nbarg
       ...
       switch argType
           case 'position'
             ...
             [bdy,d_lev,mulHeight] = wtbutilsX('utnbCFS_PREFS');
             Def_Btn_Height = mextglobX('get','Def_Btn_Height');
             if fullSIZE
             ...
             utnbcfsX('visible',fig,ud.visible);
             if nargout>0
  ...
  case 'update_NbCfs'
    ...
    switch typeUpd
       ...
       case 'anal'
          ...
          longs = wmemtoolX('rmb',fig,n_membloc0,ind_longs);
  ...
  case 'update_methode'
    ...
    if ~isempty(varargin)
        ...
        [numMeth,nameMeth] = utnbcfsX('get',fig,'numMeth','nameMeth');
    ...
    calledFUN = wfigmngrX('getWinPROP',fig,'calledFUN');
    if isequal(toolOPT,'cf1d')
        feval(calledFUN,'set_Stems_HDL',fig,'reset',nameMeth);
    end
 
    [ediActBkColor,ediInActBkColor] = ...
        mextglobX('get','Def_Edi_ActBkColor','Def_Edi_InActBkColor');
    switch nameMeth
      case 'Global'
        if resetFLAG
            set(pop_app,'Value',1);
            nbKept = utnbcfsX('get',fig,'nbOri');
        end
    ...
    switch nameMeth
      case {'Global','Manual','ByLevel'}
        ...
        set(vis_ON, 'Visible','On');
        if resetFLAG
            utnbcfsX('update_AppFlag',fig,pop_app);
            utnbcfsX('set',fig,'nbKept',nbKept);
  ...
  case 'update_AppFlag'
    if ~isempty(varargin) , uic = varargin{1}; else uic = gcbo; end
    appFlag = get(uic,'Value');
    [nameMeth,nbOri] = utnbcfsX('get',fig,'nameMeth','nbOri');

    switch nameMeth
      case {'Global','ByLevel','Manual'}
        if isequal(nameMeth,'ByLevel') && (appFlag==2)
            BkColor = mextglobX('get','Def_Edi_ActBkColor');
            ena_val = 'On';
        else
            BkColor = mextglobX('get','Def_Edi_InActBkColor');
            ena_val = 'inactive';
        end
      ...
      case {'Stepwise'}
        ...
        Nb_Coefs = nbOri(end);
        App_Len  = nbOri(1);
        Min_Val  = wstr2numX(get(edi_min_mov,'String'));
        Max_Val  = wstr2numX(get(edi_max_mov,'String'));
        switch appFlag
  ...
  case 'update_by_UIC'
    ...
    typHdl = get(obj,'Style');  
    typHdl = typHdl(1:3);
    if isequal(typHdl,'edi') || isequal(typHdl,'sli')
    ...
    switch nameMeth
      case 'Global'
        if idx>=0
            ...
            nbKept = utnbcfsX('get',fig,'nbKept');
        end
  ...
  case 'residuals'
    [handleORI,handleTHR,handleRES] = ...
        utnbcfsX('get',fig,'handleORI','handleTHR','handleRES');
    wmoreresX('create',fig,tog_res,handleRES,handleORI,handleTHR,'blocPAR');
  ...
  case {'apply','select','unselect'}
    ...
    if msgFLG && isequal(option,'select')
        ...
        if sum(nbKept)==0
            WarnString = sprintf([...
                'To select or to unselect coefficients, select an area in one of\n',...
                'lower plots by holding down the left-mouse button and dragging.\n',...
                'Then click on the select or on the unselect button.\n',...
                'You may select or unselect individual coefficients by clicking\n', ...
                'on the stems.' ...
                ]);
            wwarndlgX(WarnString,'Wavelet Toolbox Select','bloc')
        end
    end
  ...
  case 'Init_Movie'
    ...
    % Option: INIT_MOVIE %
    %--------------------%
    pop_lev = utanaparX('handles',fig,'lev');
  ...
  case 'Mngr_Movie'
    ...
    if (idx==2) || (idx==3 && ~okAuto)
        Min_Val = wstr2numX(get(edi_min_mov,'String'));
        Stp_Val = wstr2numX(get(edi_stp_mov,'String'));
        Max_Val = wstr2numX(get(edi_max_mov,'String'));
    ...
    switch idx
      ...
      case 4    % Option: CANCEL_MOVIE %
        ...
        utnbcfsX('update_methode',fig,'reset');       
    end
  ...
  switch nameMeth
    case 'Global' 
      switch toolOPT
        case 'cf1d'
          [coefs,longs] = wmemtoolX('rmb',fig,n_membloc0, ...
                                   ind_coefs,ind_longs);
          [~,nkeep] = wdcbmX(coefs,longs,3);
          nkeep  = fliplr(nkeep);
          lkeep  = length(nkeep);
          nbKept = longs;
          ...
          utnbcfsX('set',fig,'nbKept',nbKept);
          cf1dtoolX('apply',fig);
          cf1dtoolX('show_ori_sig',fig,'On');

        case 'cf2d'
          [coefs,sizes] = wmemtoolX('rmb',fig,n_membloc0, ...
                                   ind_coefs,ind_sizes);
          [~,nkeep] = wdcbm2X(coefs,sizes,1.5);
          nkeep  = fliplr(nkeep);
          lkeep  = length(nkeep);
          nbKept = utnbcfsX('get',fig,'nbKept');
          nbKept(2:1+lkeep) = nkeep;
    ...
    case 'Stepwise' 
      ...
      nbOri = utnbcfsX('get',fig,'nbOri');
      set(pop_met,'Value',numMeth);
      utnbcfsX('update_methode',fig)
      if isempty(parDemo)
          set(edi_min_mov,'String',sprintf('%.0f',1));
          utnbcfsX('update_Edi_Movie',fig,'Min');
          Min_Val = wstr2numX(get(edi_min_mov,'String'));
      ...
      set(edi_stp_mov,'String',sprintf('%.0f',Stp_Val));
      set(edi_max_mov,'String',sprintf('%.0f',Max_Val));
      utnbcfsX('update_Edi_Movie',fig,'Max')
      utnbcfsX('update_Edi_Movie',fig,'Stp')
      pause(1)
      utnbcfsX('Mngr_Movie',fig,pus_mov_sta);
  end
