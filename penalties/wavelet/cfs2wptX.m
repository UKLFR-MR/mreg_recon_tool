function [t,X] = cfs2wptX(wname,size_of_DATA,tn_of_TREE,order,CFS)
...
%     noisyT = cfs2wptX('sym4',size(X),tnodesX(t),4,noisyCfs);
%     plot(noisyT)
%
%     t = cfs2wptX('sym4',[1 1024],[3 9 10 2]',2);
%     sN = read(t,'sizes',[3,9]);
%     sN3 = sN(1,:); sN9 = sN(2,:);
%     cfsN3 = ones(sN3);
%     cfsN9 = randn(sN9);
...
[d,p] = ind2depoX(order,tn_of_TREE);
depth_of_TREE = max(d);

% Building the tree.
%-------------------
switch order
    case 2 ,t  = wpdecX(dummy_DATA,1,wname);    
    case 4 ,t  = wpdec2X(dummy_DATA,1,wname);
end
tn = leavesX(t);
