function varargout = wtbutilsX(option,varargin)
%WTBUTILS Wavelet Toolbox (Resources) Utilities.
%   OUT1 = wtbutilsX(OPTION,VARARGIN)

%   M. Misiti, Y. Misiti, G. Oppenheim, J.M. Poggi 04-May-99.
%   Last Revision: 10-Jan-2008.
...
switch option
    case 'colors'
        type = varargin{1};
        DefColor = mextglobX('get','Def_DefColor');
        ...
        switch type
            ...
            case 'ssig' % TYPE = 'ssig' - synthesized signal color.
                if blackFLG , varargout{1} = [1 1 0];       % Yellow
                else          varargout{1} = [1 0 1]/1.75;  % Dark Magenta
                    % varargout{1} = [1 1 0]/1.75;  % Dark Yellow (1)
                    % varargout{1} = [1 1 0]/1.50;  % Dark Yellow (2)
                end

            case 'app'  % TYPE = 'app'  - approximations colors.
                % in2 = NB Colors or 'flag'
                %--------------------------
                if blackFLG , varargout{1} = [0 1 1];       % Cyan
                else          varargout{1} = [0 0 1]/1.05;  % Dark Blue
                end
                if length(varargin)>1
                    if ischar(varargin{2}) , return; end
                    nbCOL = varargin{2};
                    varargout{1} = ...
                        wtbutilsX('colors','scaled',varargout{1},nbCOL);
            ...
            case 'det'  % TYPE = 'det'  - details colors.
                % in2 = NB Colors or 'flag'
                %--------------------------
                if blackFLG , varargout{1} = [0 1 0];      % Green
                else          varargout{1} = [0 0.5 0];    % Dark Green
                end
                if length(varargin)>1
                    if ischar(varargin{2}) , return; end
                    varargout{1} = ...
                        wtbutilsX('colors','scaled',varargout{1},varargin{2});
                end
            ...
            case 'stem'
                if blackFLG , varargout{1} = [1 1 0];      % Yellow
                else          varargout{1} = [0.9 0.8 0];  % Dark Yellow
                end

            case 'Col_THR_GBL'  % [col_linTHR,Col_linNOR,Col_LinZER]
                if blackFLG , varargout = {[1 1 0],[1 0 1],[0 1 1]};
                else          varargout = {[0 0 0],[1 0 1],[0 1 1]};
                end
                
            case 'linTHR'
                if blackFLG , varargout = {[1 1 0]};
                else          varargout = {[0 0 0]};
                end

            case 'stem_filters'
                if blackFLG , varargout = {[1 1 0]};   % Yellow
                else          varargout = {[0 0 1]};
                end

            case 'coefs' , varargout = {[1 1 0]};                
            case 'res'
                if blackFLG , varargout = {[1 0.70 0.28]};      % Orange
                else          varargout = {[1 0.70 0.28]/1.1};  % Dark Orange
                end
                
            case 'tree'  , varargout = {[1 1 1],[1 1 0]};  % OBSOLETE
    ...
    case 'wputils'
        axeFontSize = mextglobX('get','Def_AxeFontSize');
        type = varargin{1}(1);
    ...
    case 'title_PREFS'
        ...
        varargout = {TitColor,'bold','arial'};

    case 'dw1d_DISP_PREFS'
        if scrDIM(4)<=600 , dyLow = 0; mulH = 1;
        else                dyLow = 3*mextglobX('get','Y_Spacing'); mulH = 1.5;
        end
        varargout = {dyLow,mulH};

    case 'dw1d_DEC_PREFS'
        if scrDIM(4)<= 600 , fontsize = 11; else fontsize = 13;  end
        varargout = {fontsize};

    case {'cw1d_PREFS','cwim_PREFS'}
        [dy,btnHeight] = mextglobX('get','Y_Spacing','Def_Btn_Height');
        if scrDIM(4)<= 600
    ...
    case 'dynV_PREFS'
        if isunix , infoFontSize = 10; else infoFontSize = 8; end
        if     scrDIM(3)<= 800  , plusWidth = 1; d_WidthMax = 100;
        elseif scrDIM(3)<= 1024 , plusWidth = 2; d_WidthMax = 110;
        else                      plusWidth = 2; d_WidthMax = 110;
        end
        if scrDIM(4)<= 600 , infoFontSize = 7; end
        varargout = {plusWidth,d_WidthMax,infoFontSize};

    case 'deno1D_PREFS'
        if     scrDIM(4)<= 600 , bdy = 2; dyLEV = 1; mulHeight = 1;
        elseif scrDIM(4)<= 768 , bdy = 4; dyLEV = 4; mulHeight = 1.5;
        elseif scrDIM(4)<= 864 , bdy = 4; dyLEV = 4; mulHeight = 1.5;
        else                     bdy = 6; dyLEV = 6; mulHeight = 1.5;
        end
    ...
    case 'deno2D_PREFS'
        btnHeight = mextglobX('get','Def_Btn_Height');
        if     scrDIM(4)<= 600  , bdy = 2; dyLEV = 2;
        elseif scrDIM(4)<= 768  , bdy = 4; dyLEV = 4;
        elseif scrDIM(4)<= 1024 , bdy = 6; dyLEV = 6;
        else                      bdy = 6; dyLEV = 6;
        end
    ...
    case {'utthrgblX_PREFS','utthrwpdX_PREFS'}
        btnHeight = mextglobX('get','Def_Btn_Height');
        if     scrDIM(4)<= 600  , bdy = 2; dyLEV = 2;
        elseif scrDIM(4)<= 768  , bdy = 4; dyLEV = 4;
        elseif scrDIM(4)<= 1024 , bdy = 6; dyLEV = 6;
    ...
    case 'utnbCFS_PREFS'
        if     scrDIM(4)<= 600 , bdy = 2; dyLEV = 2;
        elseif scrDIM(4)<= 768 , bdy = 4; dyLEV = 3; mulHeight = 1.5;
        else                     bdy = 4; dyLEV = 4; mulHeight = 1.5;
        end
        if isempty(varargin) , varargout = {bdy,dyLEV,mulHeight}; return; end
        switch varargin{1}
            case 'yParams'   , varargout = {bdy,dyLEV};
    ...
    case 'utSTATS_PREFS'
        ...
        elseif scrDIM(4)<= 768
            if isunix , minSize = 10; else minSize = 8; end
        elseif scrDIM(3)<= 1024 , 
            if isunix , minSize = 10; else minSize = 8; end
        elseif scrDIM(3)<= 1280 , minSize = Inf;
        else                      minSize = Inf;
        end
        varargout = {minSize};        
        
    case 'extension_PREFS'
        if     scrDIM(3)<= 800  , deltaX = 10;
        elseif scrDIM(3)<= 1024 , deltaX = 2;            
        elseif scrDIM(3)<= 1280 , deltaX = 2;
        else                      deltaX = 2;
        end
        varargout = {deltaX};        
end
