...
function argOUT = ImportVAR_PROC_1(option)
    ...
    switch option
        case 'cfs1d'
            DlgName = 'Import DW1D Coefficients';
            FN = {'longs','coefs'};

        case 'dec1d'
            DlgName = 'Import DW1D Decomposition';
            FN = {'longs','coefs','data_name','wave_name'};

        case 'cfs2d'
            DlgName = 'Import DW2D Coefficients';
            FN = {'coefs','sizes'};

        case 'dec2d'
            DlgName = 'Import DW2D Decomposition';
            FN = {'coefs','sizes','wave_name'};

        case 'decwp1d'
            DlgName = 'Import WP1D Decomposition';
            FN = {'tree_struct','data_name'};

        case 'decwp2d'
            DlgName = 'Import WP2D Decomposition';
            FN = {'tree_struct','data_name'};

        case 'mdec1d'
            DlgName = 'Import MultiSignal Decompositions';
            FN = {'dirDec','level','wname','dwtXFilters','dwtXEXTM','dwtXShift', ...
                  'dataSize','ca','cd'};

        case 'nwav'
            DlgName = 'Import Pattern';
            FN = {'X','Y'};

        case 'reg'
            DlgName = 'Regression Estimation';
            FN = {'xdata','ydata'};
    end
    ...
    if num_of_vars>0
        ...
        [numVAR,okVAR] = listdlg('PromptString','Select data to import:',...
            'SelectionMode','single','ListString',all_var_names, ...
            'Name',DlgName,'ListSize',[180 100]);
    else
        % warndlg(xlate('No data of selected type.'),'Warning Message','modal');
        listdlg('PromptString','Select data to import:',...
            'SelectionMode','single',...
            'OKString','Close', ...
            'ListString','No data of selected type.', ...
            'Name',DlgName,'ListSize',[180 100]);
        okVAR = false;
    end
    if okVAR
...
IndexedImageOnly = wtbxmngrX('get','IndexedImageOnly');
...
switch data_type
    ...
    case '1d'
        var_type_str = {...
            ...
            'Two-Dimensional Real Data (Concatenated)'};
        pan_type_str = {'1D','2D_Concat'};
        
    case '1d_im'
        var_type_str = {'One-Dimensional Data','2D_Concat'};
        pan_type_str = {'1D_Im','2D_Concat'};
                
    case '2d'
        var_type_str = {...
            'Two-Dimensional Data or Indexed Image',...
            'True Color Image'};
        pan_type_str = {'2D','RGB'};
        if IndexedImageOnly , 
    ...
    case 'wp1d'
        var_type_str = {...
            'One-Dimensional Real Data', ...
            'Two-Dimensional Real Data (Concatenated)',... 
            'One-Dimensional Wavelet Packet Tree'};
    ...
    case 'wp2d'
        var_type_str = {...
            ...
            'True Color Image', ...            
            'Two-Dimensional Wavelet Packet Tree'};
    ...
    case 'mdw1d'
        var_type_str = {...
            'Two-Dimensional Data','Multi-Dimensional Decompositions'};
        pan_type_str = {'2D','MdwtXdec'};
        
    case 'wmul'
        var_type_str = {'Multi-Dimensional Data (nb<9)'};
        pan_type_str = {'WMUL'};
        
    case 'part'
        var_type_str = {'Partition Data'};
        pan_type_str = {'Part'};
        
    case 'mdwt2X'
        var_type_str = {...
            'Two-Dimensional Data','All Data'};
        pan_type_str = {'3D','AllMore'};
 
    case '3d'
        var_type_str = {...
            'Three-Dimensional Data','All Data'};
        pan_type_str = {'3D','AllMore'};
        
    otherwise
        var_type_str = {'All (M-by-N, M-by-N-by-3)',...
            'Binary (M-by-N logical type)',...
            'Indexed (M-by-N integer values)',...
            'Intensity (M-by-N)',...
            'Truecolor (M-by-N-by-3)',...
            'Decomposition'};
        pan_type_str = ...
...
set(wfindobjX(hImportFig),'units','normalized')
...
%--------------------------------------------------------------------------
    ...
    function createButtonPanel 
        ...
        % add buttons
        button_strs_n_tags = {'OK','okButton';'Cancel','cancelButton'};
...
%--------------------------------------------------------------------------
    function status = getVars
        ...
        if isempty(list_str)
            ...
            if isempty(all_str)
                error_str = ...
                    sprintf('There are no variables containing selected data type in the workspace.');
            else
                error_str = ...
                    sprintf('You have not selected a variable from the workspace.');
            end
            errordlg(error_str);
            status = FAILURE;
            return;
...
%	the filter specification.
  ...
  function true_or_false = isWPT(var_struct,order)
    ...
    true_or_false = isa(data,'wptree') && treeordX(data)==order; 
  end
  ...
  function true_or_false = isMultiDEC(var_struct)
    ...
    if true_or_false
        ...
        FNdec = {...
            'dirDec';'level';'wname';'dwtXFilters';'dwtXEXTM'; ...
            'dwtXShift';'dataSize';'ca';'cd'};
