function [s2,w1,w2,w3,xy] = wavefun2X(wname,in2,in3)
...
% Check arguments.
if ~isequal(wtype,1)
    errargtX(mfilename,['Invalid wavelet type'],'msg');
...
switch nargin
	...
	case 2 
        if in2 == 0 , pflag = 1; else iter = in2; end
		
	otherwise	
        pflag = 1;
        if  ischar(in2)
            if ~ischar(in3) , iter = in3; end
        else
            if ischar(in3)
                iter = in2;
            else
