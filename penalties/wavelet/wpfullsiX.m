...
switch option
    case 'full'
        ...
        %------------------
        mx = 0.06; my =0.06;
        pos_gra = wmemtoolX('rmb',win_wptool,n_wp_utils,ind_gra_area);
        ...
        if act==0
            ...
            set(btn,'String',['end ' sprintf('%.0f',num)],'Userdata',1);
            
            if old_num~=0;
               pos_param = get(txt_full,'Userdata');
               wpfullsiX('squish',win_wptool,lst_handles,txt_full,'end',old_num);
            end
            delta = zeros(1,4);
            switch num
            ...
            wpfullsiX('squish',win_wptool,lst_handles,txt_full,'beg',num,'xlab');
        else
            % end full size.
            %---------------
            ...
            set(btn,'Backgroundcolor',col,...
                    ...
                    'Userdata',0);
            wpfullsiX('squish',win_wptool,lst_handles,txt_full,'end',num,'xlab');
            usr_Pos = get(Sli_Pos,'Userdata');
            if ~isempty(usr_Pos)
                p_Pos = usr_Pos(1,:); p_Siz = usr_Pos(2,:);
                set(Sli_Pos,'Position',p_Pos);
        ...
        for k=1:length(pus_full)
            ...
            if act_old==1
                ...
                set(pus_full(k),...
                    ...
                    'String',sprintf('%.0f',k),...
                    'Userdata',0);
                wpfullsiX('squish',win_wptool,lst_handles,txt_full,'end',k);
                set([Sli_Pos Sli_Size],'Visible','on');  
                break;
            end
        end
