function varargout = meyerX(LB,UB,N,OPT)
%MEYER Meyer wavelet.
...
if tmp<0
    error('Wavelet:FunctionArgVal:Invalid_ArgVal', ...
        'The interval [LB,UB] is not valid !')
end
...
if isequal(OPT,'phi') || isequal(OPT,'two')
    ...
    phihat(int2) = cos(pi/2*meyerauxX(3/2/pi*xa(int2)-1));

    % Compute phi using non standard inverse FFT.
    [phi,t] = instdfftX(phihat,LB,UB);
end
...
if isequal(OPT,'psi') || isequal(OPT,'two')
    ...
    psihat = zeros(1,N);
    psihat(int1) = exp(i*x(int1)/2).*sin(pi/2*meyerauxX(3/2/pi*xa(int1)-1));
    psihat(int2) = exp(i*x(int2)/2).*cos(pi/2*meyerauxX(3/4/pi*xa(int2)-1));

    % Compute psi using non standard inverse FFT.
    [psi,t] = instdfftX(psihat,LB,UB);
end
