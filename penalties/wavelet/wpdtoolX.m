function varargout = wpdtoolX(option,varargin)
...
switch option
    case 'create'
		...
		% Add Help for Tool.
		%------------------
		wfighelpX('addHelpTool',win_loctool,'Wavelet Packet Dis&play','WPDI_GUI');

		% Add Help Item.
		%----------------
		wfighelpX('addHelpItem',win_loctool,'Building Wavelet Packets','WPDI_BUILDING');
		wfighelpX('addHelpItem',win_loctool,'Wavelet Packets Atoms','WPDI_ATOMS');
		...
		wfighelpX('add_ContextMenu',win_loctool,hdl_WPDI_GUI,'WPDI_GUI');
		%-------------------------------------
    ...
    case 'draw1d'
        ...
        %************************************************%
        win_loctool = varargin{1};
        [new,Wave_Fam,Wave_Nam,nb_func,prec_val] = wpdtoolX('new',win_loctool);
        if new==0 , return; end
        ...
        [wpws,xval] = wpfunX(Wave_Nam,nb_func,prec_val);
        ...
        for k= 1:nb_axes
            ...
            if     k==1 , strlab = [strlab '  - phi function -']; %#ok<AGROW>
            elseif k==2 , strlab = [strlab '  - psi function -']; %#ok<AGROW>
            end
            wtitleX(strlab,'Parent',hdl_axe(k));
        end
        ...
        % Axes attachment.
        %-----------------
        dynvtoolX('init',win_loctool,[],hdl_axe,[],[0 0]);
    ...
    case 'new'
        ...
        nb_func  = wstr2numX(strnum(ind,:));

        % varargout{1} = 0;
        [wfam,wnam,maxn,raf] = wmemtoolX('rmb',win_loctool,n_miscella,...
