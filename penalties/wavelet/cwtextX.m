function [coefs,varargout] = cwtextX(SIG,scales,WAV,varargin)
...
%     t = linspace(-1,1,512);
%     s = 1-abs(t);
%     c = cwtextX(s,1:32,'cgau4');
%     c = cwtextX(s,[64 32 16:-2:2],'morl');
%     c = cwtextX(s,[3 18 12.9 7 1.5],'db2');
%     c = cwtextX(s,1:32,'sym2','plotMode','lvl');
%     c = cwtextX(s,1:64,'sym4','plotMode','abslvl','xlim',[100 400]);
%
%     [c,Sc] = cwtextX(s,1:64,'sym4','plotMode','scal');
%     [c,Sc] = cwtextX(s,1:64,'sym4','plotMode','scalCNT');
%     [c,Sc] = cwtextX(s,1:64,'sym4','plotMode','scalCNT','extMode','sp1');
%
%     c = cwtextX(s,1:64,'sym4','plotMode','lvl','extMode','sp0');
%     c = cwtextX(s,1:64,'sym4','plotMode','lvl','extMode','sp1');
%     c = cwtextX(s,1:64,'sym4','plotMode','lvl','extMode',{'sp1','b',300});
%
%     ext = struct('Mode','sp1','Side','b','Len',300);
%     c = cwtextX(s,1:64,'sym4','plotMode','lvl','extMode',ext);
%
%     load wcantor
%     cwtextX(wcantor,(1:256),'mexh','extmode','sp0','extLen',2000, ...
%               'plotMode','absglb');
%     colormap(pink(4))
%
...
if ~isequal(extMode,'none')
    if isstruct(extMode)
        try
            ...
            error('Wavelet:FunctionArgVal:Invalid_ArgVal', ...
                'Invalid Value for Extension Mode !')
        end
    ...
    SAV_val_SIG = val_SIG;
    val_SIG = wextendX('1d',extMode,val_SIG,extLen,extSide);
    lenSIG  = length(val_SIG);
end
...
switch dim_plot
  case 'SC'
      ...
      SC = wscalogramX(typePLOT,coefs,scales,val_SIG,xSIG);
