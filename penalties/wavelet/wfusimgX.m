function varargout = wfusimgX(varargin)
...
%
%   X1 and X2 must be of same size (see WEXTEND to resize images)
%   and represent indexed images (respectively truecolor images)
%   which are m-by-n matrices (respectively m-by-n-by-3 arrays).
...
%    load mask; X1 = X;
%    load bust; X2 = X;
%    [XFUS,TXFUS,TX1,TX2] = wfusimgX(X1,X2,'db2',5,'max','max','plot'); 

%   M. Misiti, Y. Misiti, G. Oppenheim, J.M. Poggi 13-Jan-2003.
%   Last Revision: 18-Oct-2007.
...
%    load mask; X1 = X;
%    load bust; X2 = X;
%    wfusimgX('X1',X1,'X2',X2,'wname','db3','level',2);
%    [X,t,t1,t2] = wfusimgX('X1',X1,'X2',X2,'AfusMeth','max');
...
if stdINPUT
    ...
    if isempty(X1) || isempty(X2)
        error('Wavelet:FunctionArgVal:Invalid_ArgVal', ...
            'Both images X1 and X2 must be given.');
    end
...
% Decomposition.
%---------------
tIMG1 = wfustreeX(X1,level,wname);
clear X1
tIMG2 = wfustreeX(X2,level,wname);
clear X2

% Fusion.
%--------
[XFus,tFus] = wfusdecX(tIMG1,tIMG2,AfusMeth,DfusMeth);
