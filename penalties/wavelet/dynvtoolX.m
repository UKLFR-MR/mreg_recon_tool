function varargout = dynvtoolX(option,fig,varargin)
...
if isempty(find(wfindobjX('figure')==fig,1)) , return; end
format = '%20.15f';
strNumFig = sprintf(format,fig);

switch option
    case 'rmb'
        varargout{1} = wtbxappdataX('get',fig,memBlocName);

    case 'wmb'
        wtbxappdataX('set',fig,memBlocName,varargin{1});

    case 'handles'
        memB = wtbxappdataX('get',fig,memBlocName);
    ...
    case 'get'
        ...
        if strcmp(change,'oui')
            ...
            dVTmemB = mngmbtnX('delLines',fig,'All',dVTmemB);
    ...
    case 'ini_his'
        ...
        %*******************************************************%
        if nargin==3 , dynvtoolX('get',fig,0); end
        dVTmemB       = dynvtoolX('rmb',fig);
		Txt_History   = dVTmemB.handles.Txt_History;
        Pus_Hist_Init = dVTmemB.handles.Pus_Hist_Init;
        Pus_Hist_Prev = dVTmemB.handles.Pus_Hist_Prev;
    ...
    case {'zoom+','zoom-'}
        ...
        if isempty(DynV_Sel_Box)
            WarnString = {...
                'To zoom, select an area in one of plots by holding down', ...
                'the left-mouse button and dragging. Then click on the',...
                'appropriate zoom button.'};
            wwarndlgX(WarnString,'Wavelet Toolbox Zoom','bloc')
            return;
        end
    ...
    case 'center'
        ...
        mngmbtnX('delLines',fig,'All',dVTmemB);
        if isempty(DynV_Axe_Act) , return; end		
    ...
    case {'Install_V3','Install_V3_CB','create','create_V3'}
        ...
        switch option            
            ...
            case 'Install_V3_CB'
                set(handles.Pus_ZoomXPlus  ,'Callback',[mfilename '(''zoom+'',gcbf,[1 0])'])
                set(handles.Pus_ZoomXMinus ,'Callback',[mfilename '(''zoom-'',gcbf,[1 0])'])
                set(handles.Pus_ZoomYPlus  ,'Callback',[mfilename '(''zoom+'',gcbf,[0 1])'])
                set(handles.Pus_ZoomYMinus ,'Callback',[mfilename '(''zoom-'',gcbf,[0 1])'])
                set(handles.Pus_ZoomXYPlus ,'Callback',[mfilename '(''zoom+'',gcbf,[1 1])'])
                set(handles.Pus_ZoomXYMinus,'Callback',[mfilename '(''zoom-'',gcbf,[1 1])'])
                format = '%20.15f'; ediStr = sprintf(format,handles.Edi_Center);
                set(handles.Pus_CenterX    ,'Callback',['dynvtoolX(''center'',gcbf,' ediStr ',0)'])
                set(handles.Pus_CenterY    ,'Callback',['dynvtoolX(''center'',gcbf,' ediStr ',1)'])
                set(handles.Edi_Center     ,'Callback',['dynvtoolX(''edi_center'',gcbf,' ediStr ')'])                
                set(handles.Pus_Hist_Init  ,'Callback',[mfilename '(''get'',gcbf,0)'])
                set(handles.Pus_Hist_Prev  ,'Callback',[mfilename '(''get'',gcbf,-1)'])
                set(handles.Pus_Hist_Next  ,'Callback',[mfilename '(''get'',gcbf,1)'])
                set(handles.Tog_View_Axes  ,'Callback',[mfilename '(''set_dynvzaxeX'',gcbf,gcbo)']);
        end
        ...
        wfighelpX('add_ContextMenu',fig,hdl_DYNV_ZOOM,'DYNV_ZOOM');
        wfighelpX('add_ContextMenu',fig,hdl_DYNV_INFO,'DYNV_INFO');
        wfighelpX('add_ContextMenu',fig,hdl_DYNV_HIST,'DYNV_HIST');
        wfighelpX('add_ContextMenu',fig,hdl_DYNV_ZAXE,'DYNV_ZAXE');
        %-------------------------------------
    ...
    case {'init','attach','go'}
        ...
        %********************************************************%
        try 
            dynvtoolX('get',fig,0'); mngmbtnX('delLines',fig,'All'); 
        catch %#ok<CTCH>
        end;
        %------------------------ Defaults ----------------------%
        def_ZoomColor = 'r';
        ...
        if isempty(fcn_wri) , fcn_wri = 'mngcoorX'; end
        ...
        if DynV_Status && (~isempty(g_ind) || ~isempty(Act_Axes))
            ...
            action = ['mngmbtnX(''down'',' strNumFig ');'];
        ...
        if strcmp(option,'go')
            ...
            dynvtoolX('ini_his',fig,fcn_put,fcn_get);
        end

        mempos_coor = wmemutilX('add',[],fcn_wri);
        mempos_coor = wmemutilX('add',mempos_coor,par_wri);
        mempos_coor = wmemutilX('add',mempos_coor,fcn_sel);
        mempos_coor = wmemutilX('add',mempos_coor,par_sel);
        set(Edi_PosX,'UserData',mempos_coor);
        set(Edi_PosY,'UserData',mempos_coor);

        if strcmp(option,'init') , dynvtoolX('put',fig); end

        %-------------- Storage of parameters --------------%
        if isempty(g_ind) && isempty(g_cmd) , return; end
        params = [];
        params = wmemutilX('add',params,g_ind);
        params = wmemutilX('add',params,g_cmd);
        params = wmemutilX('add',params,g_act);
        params = wmemutilX('add',params,cont_xy);
        params = wmemutilX('add',params,fcn_put);
        params = wmemutilX('add',params,fcn_get);
        params = wmemutilX('add',params,fcn_wri);
        params = wmemutilX('add',params,par_wri);
        params = wmemutilX('add',params,fcn_sel);
        params = wmemutilX('add',params,par_sel);
        params = wmemutilX('add',params,cbox);
        VdynData = get(Fra_DynVTool,'UserData');
        ind = wmemutilX('ind',VdynData,params);
        if ind==1 , return; end
        max_stack = 3;
        tmp = [];
        tmp = wmemutilX('add',tmp,params);
        nb = 1; k=1;
        while (k<=max_stack+ind) && (nb<=max_stack) 
            if k~=ind
                tmp = wmemutilX('add',tmp,wmemutilX('get',VdynData,k));
                nb  = nb+1;
            end
            k = k+1;
        end
		...
		dynvtoolX('set_BtnOnOff',fig,'On','Init')
		% dynvtoolX('dynvzaxeX_BtnOnOff',fig,'On')
    ...
    case 'on'
        ...
        if ini , dynvtoolX('get',fig,0,1); end
        dVTmemB = dynvtoolX('rmb',fig);
        Fra_DynVTool = dVTmemB.handles.Fra_DynVTool;
        VdynData = get(Fra_DynVTool,'UserData');
        if ~isempty(VdynData)
            continu = 1;
            params = wmemutilX('get',VdynData);
            [g_ind,params]   = wmemutilX('get',params);
            [g_cmd,params]   = wmemutilX('get',params);
            [g_act,params]   = wmemutilX('get',params);
            [cont_xy,params] = wmemutilX('get',params);
            [fcn_put,params] = wmemutilX('get',params);
            [fcn_get,params] = wmemutilX('get',params);
            [fcn_wri,params] = wmemutilX('get',params);
            [par_wri,params] = wmemutilX('get',params);
            [fcn_sel,params] = wmemutilX('get',params);
            [par_sel,params] = wmemutilX('get',params);
            cbox             = wmemutilX('get',params);
        else
            continu = 0;
            g_ind = []; g_cmd = []; g_act = []; 
        end
        ...
        if continu
            mngmbtnX('delLines',fig,'All',dVTmemB);
            set(Fra_DynVTool,'UserData',VdynData);
            dynvtoolX('go',fig,g_ind,g_cmd,g_act,cont_xy,...
                    fcn_put,fcn_get,fcn_wri,par_wri,fcn_sel,par_sel,cbox);
        else
            dynvtoolX('attach',fig);
        end
    ...
    case 'stop'
        ...
        dynvtoolX('get',fig,0);
        if ini , dynvtoolX('ini_his',fig); end
        dynvfcn = ['mngmbtnX(''down'',' strNumFig ');'];
        winfcn  = get(fig,'WindowButtonDownFcn');
        if isequal(winfcn,dynvfcn), set(fig,'WindowButtonDownFcn',''); end
    ...
    case 'fcn_w'
        ...
        [params,VdynData] = wmemutilX('get',VdynData);
        [g_ind,params]   = wmemutilX('get',params);
        [g_cmd,params]   = wmemutilX('get',params);
        [g_act,params]   = wmemutilX('get',params);
        [cont_xy,params] = wmemutilX('get',params);
        [fcn_put,params] = wmemutilX('get',params);
        [fcn_get,params] = wmemutilX('get',params);
        [fcn_wri,params] = wmemutilX('get',params); %#ok<ASGLU>
        [par_wri,params] = wmemutilX('get',params); %#ok<ASGLU>
        [fcn_sel,params] = wmemutilX('get',params);
        [par_sel,params] = wmemutilX('get',params);
        cbox             = wmemutilX('get',params);
        fcn_wri = varargin{1};
        ...
        params = wmemutilX('add',params,par_sel);
        params = wmemutilX('add',params,cbox);
        VdynData = wmemutilX('add',VdynData,params,'top');
        set(Fra_DynVTool,'UserData',VdynData);
    ...
    case 'visible'
        ...
        switch vis
          case 'off' , dynvtoolX('stop',fig,0);
          case 'on'  , dynvtoolX('on',fig,1);
        end
    ...
    case 'dynvzaxeX_BtnOnOff'
        dVTmemB = dynvtoolX('rmb',fig);
        if isempty(dVTmemB) , return; end	
        Tog_View_Axes = dVTmemB.handles.Tog_View_Axes;
		if length(varargin)<1
			ax = wfindobjX(get(fig,'Children'), ...
    ...
    case 'edi_center'
        ...
        dynvtoolX('set_BtnOnOff',fig,ena_Val,'Center');
...
% Get Globals.
%-------------
[ediActBkColor,ediInActBkColor,fraBkColor,uicFontSize] = ...
    mextglobX('get','Def_Edi_ActBkColor',...
    'Def_Edi_InActBkColor','Def_FraBkColor','Def_UicFontSize');
...
if bitget(flgTool,1)
    action  = [mfilename '(''zoom+'',' strNumFig ',[1 0])'];
    Pus_ZoomXPlus   = uicontrol(...
        comPusProp{:},        ...
        'Position',pos_btngx, ...
        'String','X+',        ...
        'TooltipString','Zoom in X',...
        'Enable',ena_Zoom,    ...
        'Tag','Pus_ZoomXPlus',...
        'Callback',action     ...
    );
    ...
    Pus_ZoomXMinus   = uicontrol(...
        ...
        'Position',pos_btndx, ...
        'String','X-',        ...
        'TooltipString','Zoom out X',...
        'Enable',ena_Zoom,    ...
        'Tag','Pus_ZoomXMinus',...        
        'Callback',action     ...
    );
    action  = [mfilename '(''zoom+'',' strNumFig ',[0 1])'];
    ...
    Pus_ZoomYMinus   = uicontrol(...
        ...
        'Position',pos_btndy, ...
        'String','Y-',        ...
        'TooltipString','Zoom out Y',...
        'Enable',ena_Zoom,    ...
        'Tag','Pus_ZoomYMinus',...        
        'Callback',action     ...
    );
    action  = [mfilename '(''zoom+'',' strNumFig ',[1 1])'];
    Pus_ZoomXYPlus  = uicontrol(...
        comPusProp{:},        ...
        'Position',pos_btngxy,...
        'String','XY+',       ...
        'TooltipString','Zoom in XY',...
        'Enable',ena_Zoom,    ...
        'Tag','Pus_ZoomXYPlus',...                
        'Callback',action     ...
    );
...
if bitget(flgTool,2)
    ...
    cbFunc = [mfilename '(''edi_center'',' strNumFig  ...
            ',' num2mstrX(Edi_Center) ');'];    
    ...
    cbFunc = [mfilename '(''center'',' strNumFig ',' ...
            num2mstrX(Edi_Center)];
    set(Pus_CenterX,'Callback',[cbFunc ',0);']);
    set(Pus_CenterY,'Callback',[cbFunc ',1);']);
else
    Fra_Center = []; Txt_Center = [];
...
if bitget(flgTool,4)
    ...
    action = [mfilename '(''get'',' strNumFig ',0);'];
    ...
    action         = [ mfilename '(''get'',' strNumFig ',-1);'];
...
if bitget(flgTool,5)
    ...
    action = [mfilename '(''set_dynvzaxeX'',' strNumFig ',' ...
            num2mstrX(Tog_View_Axes) ');'];
...
[heightBtn,xSpacing,ySpacing] = ...
    mextglobX('get','Def_Btn_Height','X_Spacing','Y_Spacing');
[plusWidth,d_WidthMax,infoFontSize] = wtbutilsX('dynV_PREFS');
