function x = ilwt2X(varargin)
%ILWT2 Inverse 2-D liftingX wavelet transform.
%   ILWT2 performs a 2-D liftingX wavelet reconstruction
...
%   obtained by a liftingX wavelet decomposition.
...
if level>1
   level = level-1;
   a = ilwt2X(a,LS,level,'typeDEC',typeDEC);
   if isequal(typeDEC,'wp')
       h = ilwt2X(h,LS,level,'typeDEC',typeDEC);
       v = ilwt2X(v,LS,level,'typeDEC',typeDEC);
       d = ilwt2X(d,LS,level,'typeDEC',typeDEC);
   end
end
