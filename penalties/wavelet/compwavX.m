...
%      varargin to compwavX_OpeningFcn.  This calling syntax produces a
...
gui_State = struct('gui_Name',       mfilename, ...
                   ...
                   'gui_OpeningFcn', @compwavX_OpeningFcn, ...
                   'gui_OutputFcn',  @compwavX_OutputFcn, ...
                   'gui_LayoutFcn',  [], ...
                   'gui_Callback',   []);
...
%                ----------------------                                   %
% --- Executes just before compwavX is made visible.                      %
%*************************************************************************%
function compwavX_OpeningFcn(hObject,eventdata,handles,varargin)
% This function has no output args, see OutputFcn.

% Choose default command line output for compwavX
...
%*************************************************************************%
function varargout = compwavX_OutputFcn(hObject,eventdata,handles) %#ok<*INUSL>
...
wguiutilsX('setAxesTitle',handles.Axe_RunSigWave,titleSTR);
...
C_wav = cwtX({signal,stepSIG},scales,Wav);
...
%                --------------------------------                         %
%=========================================================================%
% --- Executes on button press in Compare from nwavtoolX figure.
function NwavtoolCall_Callback(hObject,eventdata,handles,Nwavetool_PARAMS) %#ok<DEFNU>

% Save nwavtoolX information in 'Userdata' field of compwavX figure.
%-----------------------------------------------------------------
if isfield(Nwavetool_PARAMS,'demoMODE')
    F1 = allchild(0);
end
compwavX('Userdata',Nwavetool_PARAMS);
if isfield(Nwavetool_PARAMS,'demoMODE')
    F2 = allchild(0);
    hFig = setdiff(F2,F1);
...
%------------------------------
wnameDEF  = 'db1';
utanaparX('Install_V3_CB',hObject,'wtype','cwtX');
...
cbcolmapX('set',hObject,'pal',{'jet',default_nbcolors});
...
dynvzaxeX('exclude',hObject,axeCur)
ColorBarVisibility(axeCur,'Off');

% Set Title in the RunSigAdapWave axes.
%--------------------------------------
titleSTR = 'Running Signal';
wguiutilsX('setAxesTitle',handles.Axe_RunSigAdapWave,titleSTR);
...
wguiutilsX('setAxesTitle',handles.Axe_AdapWaveDetect,titleSTR);

% Set Title in the pattern detection axes.
%-----------------------------------------
titleSTR = 'Wavelet';
wguiutilsX('setAxesTitle',handles.Axe_WaveDetect,titleSTR);
...
wguiutilsX('setAxesTitle',Axe,titleSTR);
...
NBC = 240;
abs_mode = 1;
C = wcodematX(C,NBC,'mat',abs_mode);
cbcolmapX('set',handles.output,'pal',{'jet',NBC});
...
cbanaparX('set',hFig,'wav',waveDEM);
