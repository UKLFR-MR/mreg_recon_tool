function varargout = cbanaparX(option,fig,varargin)
...
switch option
    case 'no_pop'
        ...
        [Pop_Wav_Fam,Pop_Wav_Num,Pop_Lev] = utanaparX('handles',fig,'pop');
        ind = wcommonX(pop_liste,[Pop_Wav_Fam,Pop_Wav_Num,Pop_Lev]);
        pop_liste(ind) = [];
    ...
    case 'cba_fam'
        ...
        tab  = wavemngrX('fields',{'fsn',strf},'tabNums');
        nbNUM = size(tab,1);
        vis = 'on';
        switch  nbNUM;
            case 0 , vis = 'off';
        ...
        for k = 1:2:nbarg
           ...
           switch argType
             ...
             case 'n_s' ,
                ...
                n_s = [argVal{1} '  (' ];
                if   dim==1
                  n_s = [n_s,int2str(siz) ')']; %#ok<AGROW>
                else
                  n_s = [n_s,int2str(siz(2)) 'x' int2str(siz(1)) ')']; %#ok<AGROW>
                end
    ...
    case 'get'
        ...
        if nbarg<1 , return; end
        [Txt_Data_NS,Edi_Data_NS,Pop_Wav_Fam,Pop_Wav_Num,Pop_Lev] = ...
            utanaparX('handles',fig); %#ok<ASGLU>
        for k = 1:nbarg
           outType = varargin{k};
           switch outType
             case 'wav'
             ...
             case 'lev'
                 ...
                 if ~isequal(levPar{1},'edit')
                     ...
                     varargout{k} = wstr2numX(levPar{2}(levPar{3},:));
                 else
                     varargout{k} = wstr2numX(levPar{2});
                 end
    ...
    case 'enable'
        ena = varargin{1};
        [Pop_Wav_Fam,Pop_Wav_Num,Pop_Lev] = utanaparX('handles',fig,'fam','num','lev');
    ...
    case 'cba_num'
        ...
        if strcmp(item,'**')
            ...
            hdl_str = num2mstrX([Pop_Wav_Fam;Pop_Wav_Num;edi_num;ena_hdl]);
            cba = [mfilename '(''ok_num'',' num2mstrX(fig) ',' hdl_str  ');'];
            set(edi_num,'Callback',cba);
            
            to_Ena_OFF_Edi_Num = wfindobjX(fig,'enable','On');
            to_Ena_OFF_Edi_Num = setdiff(to_Ena_OFF_Edi_Num,edi_num);
            wtbxappdataX('set',fig,'to_Ena_OFF_Edi_Num',to_Ena_OFF_Edi_Num);
    ...
    case 'ok_num'
        ...
        if iscell(tmp{1})
            w_fam = deblanklX(tmp{1}{tmp{2}});
        else
            w_fam = deblanklX(tmp{1}(tmp{2},:));
        end

        [typNums,w_fileName] ...
            = wavemngrX('fields',{'fsn',w_fam}, 'typNums','file');
        ...
        switch typNums
          ...
          case 'string'
            try
              bounds = wavemngrX('fields',{'fsn',w_fam}, 'bounds');
              nbpoints = 10;
              wname  = [w_fam,item];
        ...
        if ~err
            ...
            r = size(lst,1);
            if iscell(lst)
                lst = strvcat(lst{:}); %#ok<VCAT>
            end
            switch typNums
              case {'integer','real'}
                for k=1:r-1
                    num_k = wstr2numX(lst(k,:));
                    if num==num_k
                      ok  = 0;
                      val = k;
                      break;
                    elseif  num<num_k
                      ...
                      if k==1 , lst_beg = item;
                      else      lst_beg = strvcat(lst(1:k-1,:),item); %#ok<VCAT>
                      end
                      if r<rmax ,    middleLST = lst(k:r-1,:);
                      elseif k<r-1 , middleLST = lst(k:r-2,:);
                      else           middleLST = '';
                      end
                      lst = strvcat(lst_beg,middleLST,lst(r,:)); %#ok<VCAT>
                      break;
                    end
                end
                if val==0 ,
                    ...
                    item = getItem(num,typNums);
                    if r==rmax , r = r-1; end
                    lst = strvcat(lst(1:r-1,:),item,lst(r,:)); %#ok<VCAT>
                end
        ...
        if err
           wwarndlgX('Invalid Wavelet Number!','Select Wavelet Number','modal');
        end
        to_Ena_OFF_Edi_Num = wtbxappdataX('get',fig,'to_Ena_OFF_Edi_Num');
...
[i_fam,i_num,wav_fam,num_str,tab_num,add_num] = wavemngrX('indw',wname);
...
if add_num
    if lnum==1
        ...
        tab_num = strvcat(tab_num(1:lnum-1,:),num_str,tab_num(lnum,:)); %#ok<VCAT>
    end
end
if isempty(tab_num)
    tab_num = 'no'; vis  = 'off';
