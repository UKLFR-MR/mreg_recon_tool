function varargout = utentparX(option,fig,varargin)
...
switch option
    case 'create'
        ...
        for k=1:2:nbarg
            ...
            switch arg
              ...
              case 'nam' ,     ent_Nam = lower(deblanklX(varargin{k+1}));
              case 'par' ,     ent_Par = varargin{k+1};
              case 'ent'
                ent_Nam = lower(deblanklX(varargin{k+1}{1}));
		...
		%-------------------------------------
		wfighelpX('add_ContextMenu',fig,ud.handles,'WP_ENTROPY');
		%-------------------------------------
        ...
        if ~isequal(ent_Nam,'default')
            utentparX('set',fig,'ent',{ent_Nam,ent_Par});
        end
        if nargout>0
    ...
    case 'create_copy'
        ...
        [varargout{1},varargout{2}] = utentparX('create',fig,createArg{:});
        [pop_ent,uic_ent] = utentparX('handles',fig,'act');
    ...
    case 'set'
        ...
        [fra_ent,txt_ent,pop_ent,txt_par,uic_ent] = utentparX('handles',fig);
        if nbarg==0
           tmp = get(pop_ent,{'String','Value'});
           ent_Nam = lower(deblanklX(tmp{1}(tmp{2},:)));
           ent_Par = '';
        end
        for k = 1:2:nbarg
           argType = varargin{k};
           argVal  = varargin{k+1};
           switch argType
             case 'nam' , ent_Nam = lower(deblanklX(argVal));
             case 'par' , ent_Par = argVal;
             case 'ent'
               ent_Nam = lower(deblanklX(argVal{1}));
               ent_Par = argVal{2};
           end
        end
        num = getNum(ent_Nam);
    ...
    case 'get'
        ...
        ent_Nam = lower(deblanklX(ent_Nam));
        ent_Par = deblanklX(get(uic_ent,'String'));
        switch ent_Nam
