function varargout = dw1dvdrvX(option,win_dw1dtoolX,in3,in4)
...
men = findobj(win_dw1dtoolX,'type','uimenu','tag',tag_Def_DispM);
DW1D_Display_Mode = get(men,'Userdata');
if isempty(DW1D_Display_Mode) , DW1D_Display_Mode = 1; end
pop_viewm = findobj(get(win_dw1dtoolX,'Children'),'flat','tag',tag_pop_viewm);
...
switch option
   case 'plot_sig'
       ...
       dw1dscrmX('plot_sig',win_dw1dtoolX,in3,DW1D_Display_Mode,vsig);

   case 'plot_cfs'
       dw1dscrmX('plot_cfs',win_dw1dtoolX,DW1D_Display_Mode);

   case 'plot_anal'
       if DW1D_Display_Mode==1
           dw1dscrmX('plot_anal',win_dw1dtoolX);
       else
   ...
   case 'plot_synt'
       if DW1D_Display_Mode==1
           dw1dscrmX('plot_synt',win_dw1dtoolX);
       else
           set(pop_viewm,'Value',DW1D_Display_Mode);
           dw1dvmodX('ch_vm',win_dw1dtoolX,1);
       end
   ...
   case 'get_imgcfs'
        ...
        switch view_m_orig
          case {1,4}
            varargout = {'image' , findobj(win_dw1dtoolX,'tag',tag_img_cfs)};
          case {2,5}
            varargout = {'image' , []};
          case 3
            varargout = {'image' , findobj(win_dw1dtoolX,'tag',tag_img_sep)};
          case 6
            hdl = get(findobj(win_dw1dtoolX,'tag',tag_axecfsCfs),'Children');
