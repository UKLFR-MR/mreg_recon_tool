function varargout = wave2lsX(wname,factMode,varargin)
%WAVE2LS Lifting schemes associated to a wavelet.
%   LS = WAVE2LS(W,FACTMODE) returns the liftingX scheme or 
%   the cell array of liftingX schemes LS associated to the  
...
%   returns the liftingX schemes obtained from both factorization.
%
%   In addition, ... = WAVE2LS(...,PropName,Value,...) let's 
%   specify:
%      - the maximum power of the low-pass synthezis Laurent
%        polynomial (POWMAX)
%      - the difference between low-pass and high-pass synthezis
%        Laurent polynomials (DIFPOW)
%      - the tolerance value used to perform the control about 
%        liftingX scheme(s) reconstruction (TOLERANCE)
...
[Hs,Gs,Ha,Ga,PRCond,AACond] = wave2lpX(wname,powMAX,difPOW);

% Compute liftingX schemes.
if nargout>0
