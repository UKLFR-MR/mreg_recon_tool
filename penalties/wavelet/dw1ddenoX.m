...
switch option
    case 'create'
		...
		% Add Help for Tool.
		%------------------
		wfighelpX('addHelpTool',win_denoise,'Signal De-noising','DW1D_DENO_GUI');
		...
		wfighelpX('addHelpItem',win_denoise,'Variance Adaptive Thresholding','VARTHR');
        ...
        %========================================
        [Pus_EST,Tog_THR] = utthrw1dX('get',win_denoise,'pus_den','tog_thr');
        pos_Pus_EST = get(Pus_EST,'Position');
        pos_Tog_THR = get(Tog_THR,'Position');
        
        pos_Pus_SigDorC = ...
            [pos_Tog_THR(1) , pos_Pus_EST(2)-pos_Pus_EST(4)-3*dy , ...
        ...
        if isequal(viewType,'image') && (Lev_Anal<8)
            ...
            cbcolmapX('set',win_denoise,'pal',{mapName,nbColors});
        end
        ...
        %-----------------------
        sig = dw1dfileX('sig',win_caller,1);
        axeAct = axe_datas(3);
        axes(axeAct)
        ...
        utthrw1dX('set',win_denoise,'handleORI',lin_sig);
        ...
        for k = Lev_Anal:-1:1
            ...
            lin_cfs(k) = line(...
                             ...
                             'Ydata',details(k,:),...
                             'Color',col_det(k,:));
            utthrw1dX('plot_dec',win_denoise,k, ...
        ...
        wtitleX('Original details coefficients','Parent',axeAct);
        ...
        % End waiting.
        %-------------
        utthrw1dX('enable',win_denoise,'on',(1:Lev_Anal));
    ...
    case 'denoise'
        ...
        % Clear & Get Handles.
        %----------------------
        dw1ddenoX('clear_GRAPHICS',win_denoise);
        ...
        %--------------------------------------------------------
        cxc = utthrw1dX('den_M2',win_denoise,coefs,longs);
        lxc = longs;
        xc  = waverecX(cxc,longs,Wav_Name);

        % Displaying denoised signal.
        %----------------------------
        lin_den = utthrw1dX('get',win_denoise,'handleTHR');
        if ishandle(lin_den)
            ...
            utthrw1dX('set',win_denoise,'handleTHR',lin_den);
        end     
        ...
        % Memory blocks update.
        %----------------------
        thrStruct = utthrw1dX('get',win_denoise,'thrstruct');
        thrParams = {thrStruct(1:Lev_Anal).thrParams};
        wmemtoolX('wmb',win_denoise,n_thrDATA,ind_value,{xc,cxc,lxc,thrParams});
        dw1ddenoX('enable_menus',win_denoise,'on');
    ...
    case 'compute_LVL_THR'
        ...
        [numMeth,meth,alfa] = utthrw1dX('get_LVL_par',win_denoise); %#ok<ASGLU>
        [coefs,longs] = wmemtoolX('rmb',win_caller,n_coefs_longs,...
                                       ind_coefs,ind_longs);
    ...
    case 'update_LVL_meth'
        ...
        win_caller = wmemtoolX('rmb',win_denoise,n_misc_loc,ind_win_caller);
        valTHR = dw1ddenoX('compute_LVL_THR',win_denoise,win_caller);
        utthrw1dX('update_LVL_meth',win_denoise,valTHR);
    ...
    case 'clear_GRAPHICS'
        ...
        %--------------------------
        dw1ddenoX('enable_menus',win_denoise,'off');
    ...
    case 'enable_menus'
        ...
        utthrw1dX('enable_tog_res',win_denoise,enaVal);     
    ...
    case 'save_dec'
        ...
        [wave_name,data_name] = wmemtoolX('rmb',win_caller,n_param_anal, ...
                                          ind_wav_name,ind_sig_name); %#ok<NASGU>
        thrDATA = wmemtoolX('rmb',win_denoise,n_thrDATA,ind_value);
        coefs = thrDATA{2}; %#ok<NASGU>
        longs = thrDATA{3}; %#ok<NASGU>
        thrParams = thrDATA{4}; %#ok<NASGU>
    ...
    case 'close'
        ...
        switch status
          case 1
            ...
            dw1dmngrX('return_deno',win_caller,status,lin_den);
            wwaitingX('off',win_denoise);

          case 0 , dw1dmngrX('return_deno',win_caller,status);
        end
        varargout{1} = status;
